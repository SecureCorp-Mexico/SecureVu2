# Agent Instructions for SecureVu NVR
...
# endpoint or its auth dependency — outputs docs/static/securevu-api.yaml,
...
After adding, changing, or removing an endpoint (or its auth dependency), regenerate the OpenAPI spec with `python3 generate_api_auth_spec.py` so `docs/static/securevu-api.yaml` stays in sync and the endpoint's auth requirement is documented. CI enforces this via the `--check` variant; never edit that file by hand.
...
Outbound WebSocket broadcasts go through a per-recipient classifier in `securevu/comms/ws.py` that enforces camera-level access. **The classifier is fail-closed: any topic it doesn't recognize is dropped for every client.** New outbound topics must be classified there or they'll silently disappear.
...
- Main Repository: https://github.com/SecureCorp-Mexico/SecureVu2
- Home Assistant Integration: https://github.com/SecureCorp-Mexico/SecureVu2-hass-integration
