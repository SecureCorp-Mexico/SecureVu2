...
This repository holds the main SecureVu application and all of its dependencies.

Fork [SecureCorp-Mexico/SecureVu2](https://github.com/SecureCorp-Mexico/SecureVu2.git) to your own GitHub profile, then clone the forked repo to your local machine.
...
### SecureVu Home Assistant App

This repository holds the Home Assistant App, for use with Home Assistant OS and compatible installations. It is the piece that allows you to run SecureVu from your Home Assistant Supervisor tab.

Fork [SecureCorp-Mexico/SecureVu2-hass-addons](https://github.com/SecureCorp-Mexico/SecureVu2-hass-addons) to your own Github profile, then clone the forked repo to your local machine.

### SecureVu Home Assistant Integration

This repository holds the custom integration that allows your Home Assistant installation to automatically create entities for your SecureVu instance, whether you are running SecureVu as a standalone Docker container or as a [Home Assistant App](#securevu-home-assistant-app).

Fork [SecureCorp-Mexico/SecureVu2-hass-integration](https://github.com/SecureCorp-Mexico/SecureVu2-hass-integration) to your own GitHub profile, then clone the forked repo to your local machine.
...
Upon opening, you should be prompted to open the project in a remote container. This will build a container on top of the base SecureVu container with all the development dependencies installed. This ensures everyone uses a consistent development environment without the need to install any dependencies on your host machine.
...
#### 4. Run SecureVu from the command line

VS Code will start the Docker Compose file for you and open a terminal window connected to `securevu-dev`.
...
- Run `python3 -m securevu` to start the backend.
...
- All [core](#core) prerequisites _or_ another running SecureVu instance locally available
- Node.js 20

### Making changes

#### 1. Set up a SecureVu instance

The Web UI requires an instance of SecureVu to interact with for all of its data. You can either run an instance locally (recommended) or attach to a separate instance accessible on your network.

To run the local instance, follow the [core](#core) development instructions.

If you won't be making any changes to the SecureVu HTTP API, you can attach the web development server to any SecureVu instance on your network. Skip this step and go to [3a](#3a-run-the-development-server-against-a-non-local-instance).
...
- Do not make large sweeping changes. [Open a discussion on GitHub](https://github.com/SecureCorp-Mexico/SecureVu2/discussions/new) for any large or architectural ideas.
...
The docs are built using [Docusaurus v3](https://docusaurus.io). Please refer to the Docusaurus docs for more information on how to modify SecureVu's documentation.
