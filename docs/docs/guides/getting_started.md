...
If you already have an environment with Linux and Docker installed, you can continue to [Installing SecureVu](#installing-securevu) below.

If you already have SecureVu installed through Docker or through a Home Assistant App, you can continue to [Configuring SecureVu](#configuring-securevu) below.
...
This section shows how to create a minimal directory structure for a Docker installation on Debian. If you have installed SecureVu as a Home Assistant App or another way, you can continue to [Configuring SecureVu](#configuring-securevu).

### Setup directories

SecureVu will create a config file if one does not exist on the initial startup. The following directory structure is the bare minimum to get started.
...
Now you should be able to start SecureVu by running `docker compose up -d` from within the folder containing `docker-compose.yml`. On startup, an admin user and password will be created and outputted in the logs. You can see this by running `docker logs securevu`. SecureVu should now be accessible at `https://server_ip:8971` where you can login with the `admin` user and finish configuration using the Settings UI.

## Configuring SecureVu

This section assumes that you already have an environment setup as described in [Installation](../securevu/installation.md). You should also configure your cameras according to the [camera setup guide](/securevu/camera_setup). Pay particular attention to the section on choosing a detect resolution.

### Step 1: Start SecureVu

At this point you should be able to start SecureVu and a basic config will be created automatically.

### Step 2: Add a camera

Click the **Add Camera** button in <NavPath path="Settings > Global configuration > Camera management" /> to use the camera setup wizard to get your first camera added into SecureVu. See [Adding a camera with the Add Camera Wizard](../configuration/cameras.md#adding-a-camera-with-the-add-camera-wizard) for a walkthrough of each step.
...
By default, SecureVu will use a single OpenVINO detector running on the CPU.

In many cases, the integrated graphics on Intel CPUs provides sufficient performance for typical SecureVu setups. If you have an Intel processor, you can follow the configuration below.
...
Restart SecureVu and you should start seeing detections for `person`. If you want to track other objects, they can be configured in <NavPath path="Settings > Global configuration > Objects" /> or via the [configuration file reference](../configuration/advanced/reference.md).
...
If you are using YAML to configure SecureVu instead of the UI, your configuration should look similar to this now:
...
In order to review activity in the SecureVu UI, recordings need to be enabled.
...
If you only define one stream in your `inputs` and do not assign a `detect` role to it, SecureVu will automatically assign it the `detect` role. SecureVu will always decode a stream to support motion detection, Birdseye, the API image endpoints, and other features, even if you have disabled object detection with `enabled: False` in your config's `detect` section.

If you only plan to use SecureVu for recording, it is still recommended to define a `detect` role for a low resolution stream to minimize resource usage from the required stream decoding.

:::

By default, SecureVu will retain video of all tracked objects for 10 days. The full set of options for recording can be found [here](../configuration/advanced/reference.md).
