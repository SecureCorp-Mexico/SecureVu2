...
First, you will want to configure go2rtc to connect to your camera stream by adding the stream you want to use for live view in your SecureVu config file. Avoid changing any other parts of your config at this step. Note that go2rtc supports [many different stream types](https://github.com/AlexxIT/go2rtc/tree/v1.9.10#module-streams), not just rtsp.

:::tip

For the best experience, you should set the stream name under `go2rtc` to match the name of your camera so that SecureVu will automatically map it and be able to use better live view options for the camera.
...
After adding this to the config, restart SecureVu and try to watch the live stream for a single camera by clicking on it from the dashboard. It should look much clearer and more fluent than the original jsmpeg stream.

### What if my video doesn't play?

- Check Logs:
  - Access the go2rtc logs in the SecureVu UI under Logs in the sidebar.
...
- Check go2rtc Web Interface: if you don't see any errors in the logs, try viewing the camera through go2rtc's web interface.
  - Navigate to port 1984 in your browser to access go2rtc's web interface.
    - If using SecureVu through Home Assistant, enable the web interface at port 1984.
...
To access the go2rtc stream externally when utilizing the SecureVu App (for
instance through VLC), you must first enable the RTSP Restream port.
You can do this by visiting the SecureVu App configuration page within Home
...
1. If the stream you added to go2rtc is also used by SecureVu for the `record` or `detect` role, you can migrate your config to pull from the RTSP restream to reduce the number of connections to your camera as shown [here](/configuration/restream#reduce-connections-to-camera).
...
To add camera streams to Homekit SecureVu must be configured in docker to use `host` networking mode. Once that is done, you can use the go2rtc WebUI (accessed via port 1984, which is disabled by default) to share export a camera to Homekit. Any changes made will automatically be saved to `/config/go2rtc_homekit.yml`.
