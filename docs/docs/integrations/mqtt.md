...
These are the MQTT messages generated by SecureVu. The default topic_prefix is `securevu`, but can be changed in the config file.

:::info

MQTT requires a network connection to your broker. This is typically local, but will require internet if using a cloud-hosted MQTT broker. See [Network Requirements](/securevu/network_requirements#mqtt) for details.

:::

## General SecureVu Topics

### `securevu/available`

Designed to be used as an availability topic with Home Assistant. Possible message are:
"online": published once SecureVu is running and has published its initial state. Note that this is published on every connection to the broker, so it is republished if the broker restarts or the connection drops and recovers, without SecureVu itself restarting.
"stopped": published when SecureVu is stopped normally
"offline": published automatically by the MQTT broker if SecureVu disconnects unexpectedly (via MQTT Will Message)

### `securevu/restart`

Causes SecureVu to exit. Docker should be configured to automatically restart the container on exit.

### `securevu/events`

Message published for each changed tracked object. The first message is published when the tracked object is no longer marked as a false_positive. When SecureVu finds a better snapshot of the tracked object or when a zone change occurs, it will publish a message with the same id. When the tracked object ends, a final message is published with `end_time` set.
...
Returns data about each camera, its current features, and if it is detecting motion, objects, etc. Can be triggered by publishing to `securevu/onConnect`

### `securevu/profile/set`
...
Only available when notifications are enabled in the config. Not persisted across SecureVu restarts.

### `securevu/notifications/state`

Topic with current state of notifications. Published values are `ON` and `OFF`.

## SecureVu Camera Topics
...
Topic to turn SecureVu's processing of a camera on or off at runtime. Expected values are `ON` and `OFF`. The change is persisted across SecureVu restarts (see [Runtime toggle persistence](/configuration/live#runtime-toggle-persistence)). To permanently change the configured value, use **Settings → Global configuration → Camera management** in the SecureVu UI. See [Camera state](/configuration/live#camera-state) for the difference between turning a camera off and disabling it.
...
Topic to turn object detection for a camera on and off. Expected values are `ON` and `OFF`. The change is persisted across SecureVu restarts (see [Runtime toggle persistence](/configuration/live#runtime-toggle-persistence)).
...
Topic to turn audio detection for a camera on and off. Expected values are `ON` and `OFF`. The change is persisted across SecureVu restarts (see [Runtime toggle persistence](/configuration/live#runtime-toggle-persistence)).

### `securevu/<camera_name>/audio/state`

Topic with current state of audio detection for a camera. Published values are `ON` and `OFF`.

### `securevu/<camera_name>/audio_transcription/set`

Topic to turn [live audio transcription](/configuration/audio_detectors#live-transcription) for a camera on and off. Expected values are `ON` and `OFF`. Transcribed text is published to `securevu/<camera_name>/audio/transcription`.

`ON` is ignored unless audio transcription is enabled in the config for the camera. Unlike the other camera toggles, this one is not persisted across SecureVu restarts.

**NOTE:** Requires audio detection and transcription to be enabled

### `securevu/<camera_name>/audio_transcription/state`
...
Topic to turn recordings for a camera on and off. Expected values are `ON` and `OFF`. The change is persisted across SecureVu restarts (see [Runtime toggle persistence](/configuration/live#runtime-toggle-persistence)).
...
Topic to turn snapshots for a camera on and off. Expected values are `ON` and `OFF`. The change is persisted across SecureVu restarts (see [Runtime toggle persistence](/configuration/live#runtime-toggle-persistence)).
...
`ON` is ignored unless notifications are enabled in the config for the camera. This is not persisted across SecureVu restarts. It is the same control the UI labels **Suspend until restart**.
