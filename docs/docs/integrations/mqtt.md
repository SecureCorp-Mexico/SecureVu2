...
"online": published when SecureVu is running (on startup)
"offline": published after SecureVu has stopped
...
Returns data about each camera, its current features, and if it is detecting motion, objects, etc. Can be triggered by publising to `securevu/onConnect`
...
Topic to turn SecureVu's processing of a camera on and off. Expected values are `ON` and `OFF`.
