...
Not every segment of video captured by SecureVu is of the same level of interest. The people who enter your property may be a higher priority than those just walking by on the sidewalk. For this reason, SecureVu sorts **review items** by importance into **alerts** and **detections**, with a separate **Motion** category for significant motion.
...
| **Detections** | orange          | Everything else SecureVu tracked that wasn't promoted to an alert.                                                |
...
Motion Search lets you scan recorded footage for changes inside a region of interest you draw on the camera. Unlike Motion Previews, which surfaces what SecureVu's motion detector flagged in real time, Motion Search re-analyzes the saved recordings, so it can find changes that were missed (for example, an object that appeared while motion detection was paused by `lightning_threshold`, or in a region that is normally motion-masked).
...
| Field                     | Description                                                                                                                                                                                                                                                                                                                   |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Sensitivity Threshold** | Per-pixel luminance change required to count as motion inside the ROI. Behaves like SecureVu's motion detection `threshold` setting.                                                                                                                                                                                           |
...
Once running, SecureVu scans the recording segments that overlap the time range and reports timestamps where changes were detected inside the polygon, along with the percentage of the ROI that changed. Clicking a result seeks the player to that moment so you can review what happened.
...
SecureVu's main use case is to record and surface tracked objects, so Motion Search is most useful for the cases where object detection produced nothing: there is no object to find in Explore, but you suspect something happened.
...
- **An object that was never detected.** Something SecureVu doesn't have a model label for, an object too small or distant to be detected, or movement in a region where detection isn't running. The activity left no tracked object but did change the pixels, so a search can still find it.
...
When [Generative AI review](/configuration/genai/genai_review) is configured, SecureVu can generate a title, description, and threat classification for review items and surface them automatically in Review and History. Clicking the summary chip opens an **AI Analysis** dialog with the generated detail and any flagged concerns.
