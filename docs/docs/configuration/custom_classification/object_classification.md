...
- **Attribute**:
  - Added as metadata to the object, visible in the Tracked Object Details pane in Explore, `securevu/events` MQTT messages, and the HTTP API response as `<model_name>: <predicted_value>`.
...
Creating and training the model is done within the SecureVu UI using the `Classification` page. The process consists of two steps:

### Step 1: Name and Define

Enter a name for your model, select the object label to classify (e.g., `person`, `dog`, `car`), choose the classification type (sub label or attribute), and define your classes. SecureVu will automatically include a `none` class for objects that don't fit any specific category.
...
- **Preprocessing**: Ensure examples reflect object crops similar to SecureVu's boxes; keep the subject centered.
...
- Set **Per-process log level > `securevu.data_processing.real_time.custom_classification`** to `debug` for verbose classification logging
