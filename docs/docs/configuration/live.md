...
You can configure SecureVu to allow manual selection of the stream you want to view in the Live UI. For example, you may want to view your camera's substream on mobile devices, but the full resolution stream on desktop devices. Setting the `live -> streams` list will populate a dropdown in the UI's Live view that allows you to choose between the streams. This stream setting is _per device_ and is saved in your browser's local storage.
...
Using SecureVu's internal version of go2rtc is required to use this feature. You cannot specify paths in the `streams` configuration, only go2rtc stream names.
...
Cameras can be temporarily disabled through the SecureVu UI and through [MQTT](/integrations/mqtt#securevucamera_nameenabledset) to conserve system resources. When disabled, SecureVu's ffmpeg processes are terminated — recording stops, object detection is paused, and the Live dashboard displays a blank image with a disabled message. Review items, tracked objects, and historical footage for disabled cameras can still be accessed via the UI.

:::note

Disabling a camera via the SecureVu UI or MQTT is temporary and does not persist through restarts of SecureVu.
...
- **stalled**
  ...
  - What to try: This is usually indicative of the browser struggling to decode too many high-resolution streams at once. Try selecting a lower-bandwidth stream (substream), reduce the number of live streams open, improve the network connection, or lower the camera resolution. Also check your camera's keyframe (I-frame) interval — shorter intervals make playback start and recover faster. You can also try increasing the timeout value in the UI pane of SecureVu's settings.
...
2. **SecureVu shows that my live stream is in "low bandwidth mode". What does this mean?**
   ...
   - Verify your camera's H.264/AAC settings (see [SecureVu's camera settings recommendations](#camera_settings_recommendations)).
