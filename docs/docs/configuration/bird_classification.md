...
Bird classification requires a one-time internet connection to download the classification model and label map from GitHub. Once cached, models work fully offline. See [Network Requirements](/securevu/network_requirements#one-time-model-downloads) for details.

:::

## Minimum System Requirements

Bird classification runs a lightweight tflite model on the CPU, there are no significantly different system requirements than running SecureVu itself.
