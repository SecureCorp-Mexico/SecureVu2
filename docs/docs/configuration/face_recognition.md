...
Face recognition requires a one-time internet connection to download detection and embedding models from GitHub. Once cached, models work fully offline. See [Network Requirements](/securevu/network_requirements#one-time-model-downloads) for details.
...
SecureVu needs to first detect a `person` before it can detect and recognize a face.

:::

### Face Recognition

SecureVu has support for two face recognition model types:

- **small**: SecureVu will run a FaceNet embedding model to recognize faces, which runs locally on the CPU. This model is optimized for efficiency and is not as accurate.
- **large**: SecureVu will run a large ArcFace embedding model that is optimized for accuracy. It is only recommended to be run when an integrated or dedicated GPU / NPU is available.
...
Like the other real-time processors in SecureVu, face recognition runs on the camera stream defined by the `detect` role in your config. To ensure optimal performance, select a suitable resolution for this stream in your camera's firmware that fits your specific scene and requirements.
...
1. **Enable face recognition** in your configuration and restart SecureVu.
2. **Upload one face** using the **Add Face** button's wizard in the Face Library section of the SecureVu UI. Read below for the best practices on expanding your training set.
3. When SecureVu detects and attempts to recognize a face, it will appear in the **Train** tab of the Face Library, along with its associated recognition confidence.
...
For a step-by-step narrative of these best practices (and the same principles applied to state and object classification), see the [SecureVu Tips: Best Practices for Training](https://github.com/SecureCorp-Mexico/SecureVu2/discussions/21374) discussion.
...
### How SecureVu Decides Who a Person Is

Recognition does not happen one frame at a time. While a `person` is in view, SecureVu runs face recognition on many frames, not just a single frame. The final `sub_label` is decided from all of those attempts together, weighted by the area of each face (larger, closer faces count more), not from any single frame.

This has a few practical consequences:

- A handful of wrong guesses on blurry or distant frames usually do not change the result. If SecureVu sees a person as "Tom, Tom, Sam, Tom, Tom," it will still conclude the person was Tom.
...
1. Enable debug logs to see exactly what SecureVu is doing.
   - Enable debug logs for face recognition by adding `securevu.data_processing.real_time.face: debug` to your `logger` configuration. Restart SecureVu after this change.
     ...
     logger:
       ...
       logs:
         # highlight-next-line
         securevu.data_processing.real_time.face: debug
     ```
   ...
   - If you see no face-related messages at all, also add `securevu.embeddings.maintainer: debug` to confirm that the face processor was created at startup and that `person` updates are reaching it.

2. Ensure `person` is being _detected_. A `person` will automatically be scanned by SecureVu for a face. Any detected faces will appear in the Recent Recognitions tab in the SecureVu UI's Face Library.

   If you are using a SecureVu+ or `face` detecting model:
   ...
   If you are **not** using a SecureVu+ or `face` detecting model:
...
<FaqItem id="securevu-misidentified-a-face-can-i-tell-it-that-a-face-is-not-a-specific-person" question={'SecureVu misidentified a face. Can I tell it that a face is "not" a specific person?'}>
...
No, using another face recognition service will interfere with SecureVu's built in face recognition. When using double-take the sub_label feature must be disabled if the built in face recognition is also desired.
...
SecureVu does not store anything in its database related to face recognition. You can simply delete all of your faces through the SecureVu UI or remove the contents of the `/media/securevu/clips/faces` directory.
