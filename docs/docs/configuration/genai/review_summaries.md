...
Generative AI review summaries can also be toggled dynamically for a [camera via MQTT](/integrations/mqtt#securevucamera_namereview_descriptionsset).
...
By default, review summaries use preview images (cached preview frames) which have a lower resolution but use fewer tokens per image. For better image quality and more detailed analysis, configure SecureVu to extract frames directly from recordings at a higher resolution.
...
By default, review summaries are generated in English. Configure SecureVu to generate summaries in your preferred language by setting the `preferred_language` option.
