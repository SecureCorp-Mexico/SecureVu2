...
Generative AI review summaries can also be toggled dynamically for a [camera via MQTT](/integrations/mqtt/#securevucamera_namereviewdescriptionsset).
...
By default, review summaries use preview images (cached preview frames) which have a lower resolution but use fewer tokens per image. For better image quality and more detailed analysis, you can configure SecureVu to extract frames directly from recordings at a higher resolution:
...
By default, review summaries are generated in English. You can configure SecureVu to generate summaries in your preferred language by setting the `preferred_language` option:
...
For Home Assistant users, there is a built-in service (`securevu.review_summarize`) that makes it easy to request review reports as part of automations or scripts. This allows you to automatically generate daily summaries, vacation reports, or custom time period reports based on your specific needs.
