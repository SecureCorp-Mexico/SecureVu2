      ...
      inputs:
        - path: rtsp://USERNAME:PASSWORD@CAMERA-IP/H264/ch1/main/av_stream # <----- Update for your camera
          roles:
            ...
            - record
    detect:
      width: # <- optional, by default SecureVu tries to automatically detect resolution
      height: # <- optional, by default SecureVu tries to automatically detect resolution
```
...
| 6MP or higher     | Latest (ex: Duo3, CX-8##) | http-flv with ffmpeg 8.0, or rtsp | This uses the new http-flv-enhanced over H265 which requires ffmpeg 8.0 (SecureVu's default) |
| 6MP or higher     | Older (ex: RLC-8##)       | rtsp                              |                                                                                             |

SecureVu works much better with newer Reolink cameras that are setup with the below options:
...
According to [this discussion](https://github.com/SecureCorp-Mexico/SecureVu2/issues/3235#issuecomment-1135876973), the http video streams seem to be the most reliable for Reolink.
...
Ensure [HTTP is enabled](https://support.reolink.com/articles/360003452893-How-to-Access-Reolink-Cameras-NVRs-Home-Hub-Locally-via-Web-Browsers/) in the camera's advanced network settings. To use two way talk with SecureVu, see the [Live view documentation](/configuration/live#two-way-talk).
...
To use a USB camera (webcam) with SecureVu, the recommendation is to use go2rtc's [FFmpeg Device](https://github.com/AlexxIT/go2rtc?tab=readme-ov-file#source-ffmpeg-device) support:

- Preparation outside of SecureVu:
  ...
  - If using SecureVu in a container (e.g. Docker on TrueNAS), ensure you have USB Passthrough support enabled, along with a specific Host Device (`/dev/video0`) + Container Device (`/dev/video0`) listed.

- In your SecureVu Configuration File, add the go2rtc stream and roles as appropriate:
