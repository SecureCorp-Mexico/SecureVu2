...
SecureVu offers native notifications using the [WebPush Protocol](https://web.dev/articles/push-notifications-web-push-protocol) which uses the [VAPID spec](https://tools.ietf.org/html/draft-thomson-webpush-vapid) to deliver notifications to web apps using encryption.

:::info

Push notifications require internet access from the SecureVu server to the browser vendor's push service (e.g., Google FCM, Mozilla autopush). See [Network Requirements](/securevu/network_requirements#push-notifications) for details.
...
- SecureVu must be accessed via a secure `https` connection while signed in as a SecureVu user ([see the authorization docs](/configuration/authentication)).
- A supported browser must be used. Currently Chrome, Firefox, and Safari are known to be supported.
- In order for notifications to be usable externally, SecureVu must be accessible externally.
...
Once notifications are enabled, press the `Register This Device` button on all devices that you would like to receive notifications on. This will register the background worker. After this SecureVu must be restarted and then notifications will begin to be sent.

:::warning

Each registration is attached to the SecureVu user account you are signed in as, so you must register over a secure connection to the authenticated port (`8971`). Reverse proxies and tunnels should point at port `8971`.
...
Most Android phones have battery optimization settings. To get reliable Notification delivery the browser (Chrome, Firefox) should have battery optimizations disabled. If SecureVu is running as a PWA then the SecureVu app should have battery optimizations disabled as well.

## Notifications FAQ

<FaqItem id="how-do-i-debug-notifications-issues" question="How do I debug notifications issues?">

Push notifications involve SecureVu, your browser, and your browser vendor's push service, so it helps to work from the server outward.

1. Enable debug logs for the push client by adding `securevu.comms.webpush: debug` to your `logger` configuration. Restart SecureVu after this change.
   ...
   - `Sending test notification` and `Sending push notification for <camera>, review ID <id>` mean SecureVu handed the message off to the push service.
...
2. Verify the basics that most reports come down to:
   - SecureVu must be reached over `https` with a certificate your device trusts. Browsers silently refuse to register a service worker otherwise, and a self-signed certificate that is not installed as trusted on the device will fail.
   - On iOS, notifications only work when SecureVu has been installed to the Home Screen via **Share > Add to Home Screen** and opened from that icon. Safari and Chrome tabs cannot receive web push on iOS.
   - Each device must be registered individually, and SecureVu must be restarted after registering before anything can be sent, including test notifications.
   - The SecureVu server needs outbound internet access to the browser vendor's push service. See [Network Requirements](/securevu/network_requirements#push-notifications).

3. Test from the UI. Use the `Send a test notification` button in <NavPath path="Settings > Notifications" />. If the log shows `Sending test notification` but nothing arrives on the device, the problem is between the push service and your device rather than in SecureVu.
...
Push subscriptions are issued by the browser vendor and can be revoked, most often after a browser update, after clearing site data, or when a device has been offline for an extended period. When this happens the device still appears registered in SecureVu, but the push service rejects the message. The debug logs will show `Notification endpoint expired` with a `404` or `410` status.

Unregister and re-register the affected device from <NavPath path="Settings > Notifications" />, then restart SecureVu.
