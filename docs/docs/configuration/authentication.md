...
SecureVu stores user information in its database. Password hashes are generated using industry standard PBKDF2-SHA256 with 600,000 iterations. Upon successful login, a JWT token is issued with an expiration date and set as a cookie. The cookie is refreshed as needed automatically. This JWT token can also be passed in the Authorization header as a bearer token.

Users are managed in the UI under Settings > Users.

The following ports are available to access the SecureVu web UI.

| Port   | Description                                                                                                                                                                                                  |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `8971` | Authenticated UI and API. Reverse proxies should use this port.                                                                                                                                              |
| `5000` | Internal unauthenticated UI and API access. Access to this port should be limited. Intended to be used within the docker network for services that integrate with SecureVu and do not support authentication. |
...
In the event that you are locked out of your instance, you can tell SecureVu to reset the admin password and print it in the logs on next startup.
...
Constructing secure passwords and managing them properly is important. SecureVu requires a minimum length of 12 characters. For guidance on password standards see [NIST SP 800-63B](https://pages.nist.gov/800-63-3/sp800-63b.html). To learn what makes a password truly secure, read this [article](https://medium.com/peerio/how-to-build-a-billion-dollar-password-3d92568d9277).
...
Restarting SecureVu will reset the rate limits.

If you are running SecureVu behind a proxy, you will want to set `trusted_proxies` or these rate limits will apply to the upstream proxy IP address. This means that a brute force attack will rate limit login attempts from other devices and could temporarily lock you out of your instance. In order to ensure rate limits only apply to the actual IP address where the requests are coming from, you will need to list the upstream networks that you want to trust. These trusted proxies are checked against the `X-Forwarded-For` header when looking for the IP address where the request originated.

If you are running a reverse proxy in the same Docker Compose file as SecureVu, configure rate limiting and trusted proxies as follows:
...
The default session length for user authentication in SecureVu is 24 hours. This setting determines how long a user's authenticated session remains active before a token refresh is required. Otherwise, the user will need to log in again.
...
The JWT token secret needs to be kept secure. Anyone with this secret can generate valid JWT tokens to authenticate with SecureVu. This should be a cryptographically random string of at least 64 characters.
...
SecureVu looks for a JWT token secret in the following order:

1. An environment variable named `SECUREVU_JWT_SECRET`
2. A file named `SECUREVU_JWT_SECRET` in the directory specified by the `CREDENTIALS_DIRECTORY` environment variable (defaults to the Docker Secrets directory: `/run/secrets/`)
3. A `jwt_secret` option from the Home Assistant App options
4. A `.jwt_secret` file in the config directory

If no secret is found on startup, SecureVu generates one and stores it in a `.jwt_secret` file in the config directory.

Changing the secret will invalidate current tokens.

## Proxy configuration

SecureVu can be configured to leverage features of common upstream authentication proxies such as Authelia, Authentik, oauth2_proxy, or traefik-forward-auth. SecureVu does not implement OIDC, SAML, or LDAP natively; as an NVR focused on recording and object detection, it relies on robust, battle-tested proxies to handle those protocols and passes the authenticated user and role through via headers (see below).

If you are leveraging the authentication of an upstream proxy, you likely want to disable SecureVu's authentication as there is no correspondence between users in SecureVu's database and users authenticated via the proxy. Optionally, if communication between the reverse proxy and SecureVu is over an untrusted network, you should set an `auth_secret` in the `proxy` config and configure the proxy to send the secret value as a header named `X-Proxy-Secret`. Assuming this is an untrusted network, you will also want to [configure a real TLS certificate](tls.md) to ensure the traffic can't simply be sniffed to steal the secret.

To disable SecureVu's authentication and ensure requests come only from your known proxy:
...
If you have disabled SecureVu's authentication and your proxy supports passing a header with authenticated usernames and/or roles, you can use the `header_map` config to specify the header name so it is passed to SecureVu. For example, the following will map the `X-Forwarded-User` and `X-Forwarded-Groups` values. Header names are not case sensitive. Multiple values can be included in the role header. SecureVu expects that the character separating the roles is a comma, but this can be specified using the `separator` config entry.
...
SecureVu supports `admin`, `viewer`, and custom roles (see below). When using port `8971`, SecureVu validates these headers and subsequent requests use the headers `remote-user` and `remote-role` for authorization.
...
In some environments, upstream identity providers (OIDC, SAML, LDAP, etc.) do not pass a SecureVu-compatible role directly, but instead pass one or more group claims. To handle this, SecureVu supports a `role_map` that translates upstream group names into SecureVu's internal roles (`admin`, `viewer`, or custom). This is configurable via YAML in the configuration file:
...
- If no mapping matches, SecureVu falls back to `default_role` if configured.
- If `role_map` is not defined, SecureVu assumes the role header directly contains `admin`, `viewer`, or a custom role name.

**Note on matching semantics:**

- Admin precedence: if the `admin` mapping matches, SecureVu resolves the session to `admin` to avoid accidental downgrade when a user belongs to multiple groups (for example both `admin` and `viewer` groups).

:::note

If a user isn't getting the role you expect, enable debug logging to see exactly what headers SecureVu is receiving from your proxy:

```yaml
logger:
  default: info
  logs:
    securevu.api.auth: debug
```
...
SecureVu gracefully performs login page redirection that should work with most authentication proxies. If your reverse proxy returns a `Location` header on `401`, `302`, or `307` unauthorized responses, SecureVu's frontend will automatically detect it and redirect to that URL.
...
SecureVu supports user roles to control access to certain features in the UI and API, such as managing users or modifying configuration settings. Roles are assigned to users in the database or through proxy headers and are enforced when accessing the UI or API through the authenticated port (`8971`).
...
To use role-based access control, you must connect to SecureVu via the **authenticated port (`8971`)** directly or through a reverse proxy.
...
To use the SecureVu API, you need to authenticate first. Follow these steps to obtain a Bearer token:
...
You may need to include `-k` in the argument list in these steps (eg: `curl -k -i -X POST ...`) if your SecureVu instance is using a self-signed certificate.
