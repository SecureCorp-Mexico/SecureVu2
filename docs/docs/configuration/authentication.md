...
In the event that you are locked out of your instance, you can tell SecureVu to reset the admin password and print it in the logs on next startup using the `reset_admin_password` setting in your config file.
...
If you are running a reverse proxy in the same Docker Compose file as SecureVu, here is an example of how your auth config might look:
...
SecureVu can be configured to leverage features of common upstream authentication proxies such as Authelia, Authentik, oauth2_proxy, or traefik-forward-auth.
...
Here is an example of how to disable SecureVu's authentication and also ensure the requests come only from your known proxy.
...
In some environments, upstream identity providers (OIDC, SAML, LDAP, etc.) do not pass a SecureVu-compatible role directly, but instead pass one or more group claims. To handle this, SecureVu supports a `role_map` that translates upstream group names into SecureVu’s internal roles (`admin`, `viewer`, or custom).
