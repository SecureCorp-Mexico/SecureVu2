...
mqtt:
  ...
  enabled: True
  # Required: host name
  # NOTE: MQTT host can be specified with an environment variable or docker secrets that must begin with 'SECUREVU_'.
  #       e.g. host: '{SECUREVU_MQTT_HOST}'
...
# NOTE: Can be overridden at the camera level
snapshots:
  # Optional: Enable writing snapshot images to /media/securevu/clips (default: shown below)
  enabled: False
...
lpr:
  ...
  enhancement: 0
  # Optional: Save plate images to /media/securevu/clips/lpr for debugging purposes (default: shown below)
...
cameras:
  ...
  back:
    ...
    # (except Camera Management). Re-enabling requires a SecureVu restart.
    enabled: True
    ...
    onvif:
      ...
      #       that must begin with 'SECUREVU_'. e.g. host: '{SECUREVU_ONVIF_USERNAME}'
      host: 0.0.0.0
