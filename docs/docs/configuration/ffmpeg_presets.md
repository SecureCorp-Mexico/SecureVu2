...
Some presets of FFmpeg args are provided by default to make the configuration easier. All presets can be seen in [this file](https://github.com/SecureCorp-Mexico/SecureVu2/blob/master/securevu/ffmpeg_presets.py).

### Hwaccel Presets

It is highly recommended to use hwaccel presets in the config. These presets not only replace the longer args, but they also give SecureVu hints of what hardware is available and allows SecureVu to make other optimizations using the GPU such as when encoding the birdseye restream or when scaling a stream that has a size different than the native stream size.
...
See [the camera specific docs](/configuration/camera_specific.md) for more info on non-standard cameras and recommendations for using them in SecureVu.
...
| preset-rtsp-restream             | RTSP Stream from restream | Use for rtsp restream as source for securevu                                                      |
| preset-rtsp-restream-low-latency | RTSP Stream from restream | Use for rtsp restream as source for securevu to lower latency, may cause issues with some cameras |
