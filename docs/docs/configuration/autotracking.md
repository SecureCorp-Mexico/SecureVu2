...
A fast [detector](object_detectors.md) is recommended. CPU detectors will not perform well or won't work at all. You can watch SecureVu's debug viewer for your camera to see a thicker colored box around the object currently being autotracked.
...
Watching SecureVu's debug view can help to determine a possible cause. The autotracked object will have a thicker colored box around it.
...
### I tried calibrating my camera, but the logs show that it is stuck at 0% and SecureVu is not starting up.

This is often caused by the same reason as above - the `MoveStatus` ONVIF parameter is not changing due to a bug in your camera's firmware. Also, see the note above: SecureVu's web UI and all other cameras will be unresponsive while calibration is in progress. This is expected and normal. But if you don't see log entries every few seconds for calibration progress, your camera is not compatible with autotracking.
...
### SecureVu reports an error saying that calibration has failed. Why?
