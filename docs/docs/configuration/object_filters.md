...
There are several types of object filters that can be used to reduce [false positive](/securevu/glossary#false-positive) rates.
...
In frame 2, the score is below the `min_score` value, so SecureVu ignores it and it becomes a 0.0. The computed score is the median of the score history (padding to at least 3 values), and only when that computed score crosses the `threshold` is the object marked as a true positive. That happens in frame 4 in the example.

The **top score** is the highest computed score the tracked object has ever reached during its lifetime. Because the computed score rises and falls as new frames come in, the top score can be thought of as the peak confidence SecureVu had in the object. In SecureVu's UI (such as the Tracking Details pane in Explore), you may see all three values:
