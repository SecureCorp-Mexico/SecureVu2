...
SecureVu provides a builtin audio detector which runs on the CPU. Compared to object detection in images, audio detection is a relatively lightweight operation so the only option is to run the detection on a CPU.
...
The audio detector uses volume levels in the same way that motion in a camera feed is used for object detection. This means that SecureVu will not run audio detection unless the audio volume is above the configured level in order to reduce resource usage. Audio levels can vary widely between camera models so it is important to run tests to see what volume levels are. The [Debug view](/usage/live#the-single-camera-view) in the SecureVu UI has an Audio tab for cameras that have the `audio` role assigned where a graph and the current levels are displayed. The `min_volume` parameter should be set to the minimum the `RMS` level required to run audio detection.
...
The included audio model has over [500 different types](https://github.com/SecureCorp-Mexico/SecureVu2/blob/dev/audio-labelmap.txt) of audio that can be detected, many of which are not practical. By default `bark`, `fire_alarm`, `speech`, and `yell` are enabled but these can be customized.
...
The labelmap includes hundreds of sound types. The labels below are the ones most users may find practical, grouped by what they're typically used for. Use the exact label string from the left column in your `listen` config, or search for the label in the SecureVu UI directly.
...
Frequently-heard labels like `speech` can generate a lot of events, and each event could save a snapshot and recording based on your configuration, so start with a focused set and expand from there. The defaults (`bark`, `fire_alarm`, `speech`, `yell`) plus a few of the safety labels above cover most needs. See the [full audio labelmap](https://github.com/SecureCorp-Mexico/SecureVu2/blob/dev/audio-labelmap.txt) or the SecureVu UI for every available type.

:::

### Audio Transcription

SecureVu supports fully local audio transcription using either `sherpa-onnx` or OpenAI's open-source Whisper models via `faster-whisper`. The goal of this feature is to support Semantic Search for `speech` audio events. SecureVu is not intended to act as a continuous, fully-automatic speech transcription service. Automatically transcribing all speech (or queuing many audio events for transcription) requires substantial CPU (or GPU) resources and is impractical on most systems. For this reason, transcriptions for events are initiated manually from the UI or the API rather than being run continuously in the background.

:::info

Audio transcription requires a one-time internet connection to download the Whisper or Sherpa-ONNX model on first use. Once cached, transcription runs fully offline. See [Network Requirements](/securevu/network_requirements#one-time-model-downloads) for details.
...
The single camera Live view in the SecureVu UI supports live transcription of audio for streams defined with the `audio` role. Use the Enable/Disable Live Audio Transcription button/switch to toggle transcription processing, or toggle it outside of the UI with the [`securevu/<camera_name>/audio_transcription/set`](/integrations/mqtt#securevucamera_nameaudio_transcriptionset) MQTT topic or the HTTP API. When speech is heard, the UI will display a black box over the top of the camera stream with text. The MQTT topic `securevu/<camera_name>/audio/transcription` will also be updated in real-time with transcribed text.
...
Only one `speech` event may be transcribed at a time. SecureVu does not automatically transcribe `speech` events or implement a queue for long-running transcription model inference.
...
1. Why doesn't SecureVu automatically transcribe all `speech` events?
   ...
   Other options are being considered for future versions of SecureVu to add transcription options that support external `whisper` Docker containers. A single transcription service could then be shared by SecureVu and other applications (for example, Home Assistant Voice), and run on more powerful machines when available.
...
2. Why don't you save live transcription text and use that for `speech` events?
   ...
   Automatically persisting that data would often result in **misaligned, partial, or irrelevant transcripts**, while still incurring all of the CPU, storage, and privacy costs of transcription. That's why SecureVu treats transcription as an **explicit, user-initiated action** rather than an automatic side-effect of every `speech` event.
