...
The included audio model has over [500 different types](https://github.com/SecureCorp-Mexico/SecureVu2/blob/dev/audio-labelmap.txt) of audio that can be detected, many of which are not practical. By default `bark`, `fire_alarm`, `scream`, `speech`, and `yell` are enabled but these can be customized.
...
The single camera Live view in the SecureVu UI supports live transcription of audio for streams defined with the `audio` role. Use the Enable/Disable Live Audio Transcription button/switch to toggle transcription processing. When speech is heard, the UI will display a black box over the top of the camera stream with text. The MQTT topic `securevu/<camera_name>/audio/transcription` will also be updated in real-time with transcribed text.
