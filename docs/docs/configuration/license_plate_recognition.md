...
SecureVu can recognize license plates on vehicles and automatically add the detected characters to the `recognized_license_plate` field or a [known](#matching) name as a `sub_label` to tracked objects of type `car`, `motorcycle`, `bus`, `truck`, `school_bus`, or `garbage_truck`, depending on which of those labels your model detects. A common use case may be to read the license plates of cars pulling into a driveway or cars passing by on a street.

LPR works best when the license plate is clearly visible to the camera. For moving vehicles, SecureVu continuously refines the recognition process, keeping the most confident result. When a vehicle becomes stationary, LPR continues to run for a short time after to attempt recognition.

:::info

License plate recognition requires a one-time internet connection to download OCR and detection models from GitHub. Once cached, models work fully offline. See [Network Requirements](/securevu/network_requirements#one-time-model-downloads) for details.
...
- Published via the `securevu/events` MQTT topic as a `sub_label` ([known](#matching)) or `recognized_license_plate` (unknown) for the vehicle tracked object.
- Published via the `securevu/tracked_object_update` MQTT topic with `name` (if [known](#matching)) and `plate`.

## Model Requirements

Users running a SecureVu+ model (or any custom model that natively detects license plates) should ensure that `license_plate` is added to the [list of objects to track](https://docs.secure.vu/plus/#available-label-types) either globally or for a specific camera. This will improve the accuracy and performance of the LPR model.

Users without a model that detects license plates can still run LPR. SecureVu uses a lightweight YOLOv9 license plate detection model that can be configured to run on your CPU or GPU. In this case, you should _not_ define `license_plate` in your list of objects to track.

:::note

In the default mode, SecureVu's LPR needs to first detect a vehicle before it can recognize a license plate. If you're using a dedicated LPR camera and have a zoomed-in view where a vehicle will not be detected, you can still run LPR, but the configuration parameters will differ from the default mode. See the [Dedicated LPR Cameras](#dedicated-lpr-cameras) section below.
...
Like other enrichments in SecureVu, LPR **must be enabled globally** to use the feature. Disable it for specific cameras at the camera level if you don't want to run LPR on cars on those cameras.
...
For non-dedicated LPR cameras, ensure that your camera is configured to detect vehicle objects, and that a vehicle is actually being detected by SecureVu. Otherwise, LPR will not run. The object types that can carry a plate are defined by your model's `attributes_map`, so if your model detects other vehicle labels, you can add them there.

Like the other real-time processors in SecureVu, license plate recognition runs on the camera stream defined by the `detect` role in your config. To ensure optimal performance, select a suitable resolution for this stream in your camera's firmware that fits your specific scene and requirements.
...
- **Detection threshold**: License plate object detection confidence score required before recognition runs. This field only applies to the standalone license plate detection model; `threshold` and `min_score` object filters should be used for models like SecureVu+ that have license plate detection built in.
  - Default: `0.7`
...
- **Save debug plates**: Set to on to save captured text on plates for debugging. These images are stored in `/media/securevu/clips/lpr`, organized into subdirectories by `<camera>/<event_id>`, and named based on the capture timestamp.
...
The saved images are not full plates but rather the specific areas of text detected on the plates. It is normal for the text detection model to sometimes find multiple areas of text on the plate. Use them to analyze what text SecureVu recognized and how image enhancement affects detection.

**Note:** SecureVu does **not** automatically delete these debug images. Once LPR is functioning correctly, you should disable this option and manually remove the saved files to free up storage.
...
If a camera is configured to detect vehicles but you don't want SecureVu to run LPR for that camera, disable LPR at the camera level:
...
SecureVu's dedicated LPR mode is optimized for cameras with a narrow field of view, specifically positioned and zoomed to capture license plates exclusively. If your camera provides a general overview of a scene rather than a tightly focused view, this mode is not recommended.

:::

Users can configure SecureVu's dedicated LPR mode in two different ways depending on whether a SecureVu+ (or native `license_plate` detecting) model is used:

### Using a SecureVu+ (or Native `license_plate` Detecting) Model

Users running a SecureVu+ model (or any model that natively detects `license_plate`) can take advantage of `license_plate` detection. This allows license plates to be treated as standard objects in dedicated LPR mode, meaning that alerts, detections, snapshots, and other SecureVu features work as usual, and plates are detected efficiently through your configured object detector.
...
- License plates are treated as normal objects in SecureVu.
- Scores, alerts, detections, and snapshots work as expected.
- Snapshots will have license plate bounding boxes on them.
- The `securevu/events` MQTT topic will publish tracked object updates.
- Debug view will display `license_plate` bounding boxes.
- If you are using a SecureVu+ model and want to submit images from your dedicated LPR camera for model training and fine-tuning, annotate both the vehicle and the `license_plate` in the snapshots on the SecureVu+ website, even if the vehicle is barely visible.

### Using the Secondary LPR Pipeline (Without SecureVu+)

If you are not running a SecureVu+ model, you can use SecureVu's built-in secondary dedicated LPR pipeline. In this mode, SecureVu bypasses the standard object detection pipeline and runs a local license plate detector model on the full frame whenever motion activity occurs.
...
| Field                       | Description                                                                                                                  |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| **Enable object detection** | Set to off to disable SecureVu's standard object detection pipeline                                                           |
| **Detect FPS**              | Set to `5`. Increase if necessary, though high values may slow down SecureVu's enrichments pipeline and use considerable CPU. |
...
| Field                | Description                                                                            |
| -------------------- | -------------------------------------------------------------------------------------- |
| **Objects to track** | Set to an empty list, required when not using a SecureVu+ model for dedicated LPR mode |
...
cameras:
  dedicated_lpr_camera:
    ...
    ffmpeg: ... # add your streams
    detect:
      enabled: False # disable SecureVu's standard object detection pipeline
      fps: 5 # increase if necessary, though high values may slow down SecureVu's enrichments pipeline and use considerable CPU
      width: 1920
      height: 1080
    objects:
      track: [] # required when not using a SecureVu+ model for dedicated LPR mode
    motion:
      threshold: 30
...
- The standard object detection pipeline is bypassed. Any detected license plates on dedicated LPR cameras are treated similarly to manual events in SecureVu. You must **not** specify `license_plate` as an object to track.
...
- The `securevu/events` MQTT topic will **not** publish tracked object updates with the license plate bounding box and score, though `securevu/reviews` will publish if recordings are enabled. If a plate is recognized as a [known](#matching) plate, publishing will occur with an updated `sub_label` field. If characters are recognized, publishing will occur with an updated `recognized_license_plate` field.
...
| Feature                 | Native `license_plate` detecting Model (like SecureVu+) | Secondary Pipeline (without native model or SecureVu+)           |
| ----------------------- | ------------------------------------------------------ | --------------------------------------------------------------- |
...
| Object Detection        | Standard SecureVu+ detection applies                    | Bypasses standard object detection                              |
| Debug View              | May show `license_plate` bounding boxes                | May **not** show `license_plate` bounding boxes                 |
| MQTT `securevu/events`   | Publishes tracked object updates                       | Publishes limited updates                                       |
| Explore                 | Recognized plates available in More Filters            | Recognized plates available in More Filters                     |

By selecting the appropriate configuration, users can optimize their dedicated LPR cameras based on whether they are using a SecureVu+ model or the secondary LPR pipeline.
...
- For non-SecureVu+ users, you may need to change your camera settings for a clearer image or decrease your global `recognition_threshold` config if your plates are not being accurately recognized at night.
...
If you are using a SecureVu+ model or a custom model that detects license plates, ensure that `license_plate` is added to your list of objects to track.
If you are using the free model that ships with SecureVu, you should _not_ add `license_plate` to the list of objects to track.
...
In normal LPR mode, SecureVu requires a vehicle to be detected first before recognizing a license plate. If you have a dedicated LPR camera, you can change the camera `type` to `"lpr"` to use the Dedicated LPR Camera algorithm. This comes with important caveats, though. See the [Dedicated LPR Cameras](#dedicated-lpr-cameras) section above.
...
LPR, like other SecureVu enrichments, runs at the camera level rather than the zone level. While you can't restrict LPR to specific zones directly, you can control when recognition runs by setting a `min_area` value to filter out smaller detections.
...
1. Start with a simplified LPR config.
   - Remove or comment out everything in your LPR config, including `min_area`, `min_plate_length`, `format`, `known_plates`, or `enhancement` values so that the only values left are `enabled` and `debug_save_plates`. This will run LPR with SecureVu's default values.
...
2. Enable debug logs to see exactly what SecureVu is doing.
   - Enable debug logs for LPR by adding `securevu.data_processing.common.license_plate: debug` to your `logger` configuration. These logs are _very_ verbose, so only keep this enabled when necessary. Restart SecureVu after this change.
     ...
     logger:
       ...
       logs:
         # highlight-next-line
         securevu.data_processing.common.license_plate: debug
     ```

3. Ensure your plates are being _detected_.

   If you are using a SecureVu+ or `license_plate` detecting model:
   - Watch the [Debug view](/usage/live#the-single-camera-view) to ensure that `license_plate` is being detected.
   - View MQTT messages for `securevu/events` to verify detected plates.
   ...
   If you are **not** using a SecureVu+ or `license_plate` detecting model:
...
4. Ensure the characters on detected plates are being _recognized_.
   ...
   - Enable `debug_save_plates` to save images of detected text on plates to the clips directory (`/media/securevu/clips/lpr`). Ensure these images are readable and the text is clear.
...
<FaqItem id="i-am-seeing-a-yolov9-plate-detection-metric-in-enrichment-metrics-but-i-have-a-securevu-or-custom-model-that-detects-license_plate-why-is-the-yolov9-model-running" question={<>I am seeing a YOLOv9 plate detection metric in Enrichment Metrics, but I have a SecureVu+ or custom model that detects <code>license_plate</code>. Why is the YOLOv9 model running?</>}>
...
<FaqItem id="it-looks-like-securevu-picked-up-my-cameras-timestamp-or-overlay-text-as-the-license-plate-how-can-i-prevent-this" question="It looks like SecureVu picked up my camera's timestamp or overlay text as the license plate. How can I prevent this?">

This could happen if vehicles travel close to your camera's timestamp or overlay text. You could either move the text through your camera's firmware, or apply a mask to it in SecureVu.
