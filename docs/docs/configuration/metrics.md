...
Metrics are available at `/api/metrics` by default. No additional SecureVu configuration is required to expose them.
...
These gauges report the operating system's figures for the whole filesystem (the same numbers as `df`), not SecureVu's own recording footprint. For how this differs from the recordings usage shown in the UI, see [Understanding storage usage](/configuration/record#understanding-storage-usage).
