...
Recordings can be enabled and are stored at `/media/securevu/recordings`. The folder structure for the recordings is `YYYY-MM-DD/HH/<camera_name>/MM.SS.mp4` in **UTC time**. These recordings are written directly from your camera stream without re-encoding. Each camera supports a configurable retention policy in the config. SecureVu chooses the largest matching retention value between the recording retention and the tracked object retention when determining if a recording should be removed.
...
If there is less than an hour left of storage, the oldest hour of recordings will be deleted and a message will be printed in the SecureVu logs. This emergency cleanup deletes the oldest recordings first regardless of retention settings to reclaim space as quickly as possible.
...
In some cases the recordings files may be deleted but SecureVu will not know this has happened. Recordings sync can be enabled which will tell SecureVu to check the file system and delete any db entries for files which don't exist.
...
This feature is meant to fix variations in files, not completely delete entries in the database. If you delete all of your media, don't use `sync_recordings`, just stop SecureVu, delete the `securevu.db` database, and restart.
