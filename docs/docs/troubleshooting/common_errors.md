...
This page is an index of error messages you might see in SecureVu's logs, what each one means, and where to go next. It is organized by the kind of problem, not by which component logged the message.

Two things to know before you start:

- **Many of these messages come from FFmpeg, go2rtc, GPU drivers, or the operating system, not from SecureVu itself.** SecureVu captures and re-logs their output, so the log level shown in the SecureVu UI does not always reflect the original severity.
- **Wrapped errors put the real cause on the next line.** When SecureVu logs a generic message like `Error occurred when attempting to maintain recording cache`, the actual exception is logged immediately after it. When a camera's FFmpeg process exits, SecureVu logs `The following ffmpeg logs include the last 100 lines prior to exit` and dumps that camera's FFmpeg output. Always read those lines, they are where the answer usually is.
...
FFmpeg is running but has stopped delivering video for 20 seconds, so SecureVu's camera watchdog restarts it. The stream connected at least once, then went quiet: a camera reboot, a network drop, the camera evicting the connection, or a stalled decoder. If it repeats on a loop, the stream is unstable.

</FaqItem>

<FaqItem id="ffmpeg-process-crashed-unexpectedly" question="Ffmpeg process crashed unexpectedly for <camera>">

The detect FFmpeg process exited on its own. This message is only the notification; the cause is in the 100 FFmpeg log lines SecureVu dumps right after it (look for a `Failed to sync surface`, `Connection refused`, codec, or audio error in that block). Related watchdog messages include `<camera> exceeded fps limit`, which means the camera is delivering frames faster than `detect.fps` (usually a camera whose real frame rate differs from what is configured).
...
SecureVu's record watchdog is restarting the record FFmpeg process because the camera stopped producing usable recordings. The wording distinguishes the cases: `No new recording segments` means no new segment file reached the cache, so ffmpeg isn't getting video out of the record stream; the two `valid` variants mean recordings are arriving but keep failing validation. Either way the fault is on the camera or network side, and the restart is SecureVu trying to recover.
...
A VAAPI/QSV hardware frame-sync failure between FFmpeg and the GPU driver, not a SecureVu bug. It usually appears when the detect stream is being scaled or decoded on the GPU.
...
This comes from the NVIDIA container runtime while starting the container, not from SecureVu, and the container never starts. The NVIDIA driver is not loaded on the host. Confirm `nvidia-smi` works on the host itself (not inside the container) before troubleshooting SecureVu. In a VM or LXC, the driver must be available inside the guest. See [Hardware: Nvidia GPU](/configuration/hardware_acceleration_video).
...
The process was killed by the CPU for executing an unsupported instruction. There are two distinct causes in SecureVu:
...
SecureVu ran out of shared memory (`/dev/shm`). The container's `shm_size` is too small for the number and resolution of your detect streams, or you added cameras after startup without increasing it.

See [Calculating required shm-size](/securevu/installation#calculating-required-shm-size). If you cannot increase `shm_size`, lowering the `SHM_MAX_FRAMES` environment variable reduces how many frames SecureVu buffers per camera.

</FaqItem>

<FaqItem id="errno-28-no-space-left" question="[Errno 28] No space left on device">

A filesystem is full: the recordings volume (`/media/securevu`), the cache tmpfs (`/tmp/cache`), or `/dev/shm`. Check which one, and note that inode exhaustion can produce this while `df -h` still shows free space.
...
SQLite could not acquire the write lock. SecureVu's timeout already scales with camera count, so under normal local-disk operation this essentially only happens when the database is on a network share (SMB/NFS), where file locking is unreliable, or when two instances point at the same file.
...
The SQLite database file is corrupted, typically after hard power loss, a network-share database, or a filesystem with unsafe write semantics. SecureVu does not repair it automatically, but the database can usually be recovered by hand.

**Stop SecureVu first**, then work on the database file directly (by default `/config/securevu.db`). Start by checking what is actually wrong:

```bash
sqlite3 securevu.db "PRAGMA integrity_check;"
```
...
sqlite3 securevu.db .dump > securevu.dump

# keep the corrupt file, then rebuild from the dump
mv securevu.db securevu.db.bak
cat securevu.dump | sqlite3 securevu.db
...
As a last resort, stop SecureVu, delete `securevu.db`, and restart. SecureVu recreates it, but existing recordings lose all of their metadata. If a `backup.db` exists next to your database, SecureVu wrote it before the last schema migration and restoring it recovers everything up to that point.
