...
## I have SecureVu configured for motion recording only, but it still seems to be recording even with no motion. Why?
...
If CPU, RAM, disk throughput, or bus I/O is insufficient, nothing inside securevu will help. It is important to review each aspect of available system resources.
...
If the storage is working quickly then this error may be caused by CPU load on the machine being too high for SecureVu to have the resources to keep up. Try temporarily shutting down other services to see if the issue improves.
