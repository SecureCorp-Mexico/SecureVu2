...
SecureVu generally [recommends cameras with configurable sub streams](/securevu/hardware.md). However, if your camera does not have a sub stream that a suitable resolution, the main stream can be resized.
...
If you see repeated "On connect called" messages in your logs, check for another instance of SecureVu. This happens when multiple SecureVu containers are trying to connect to MQTT with the same `client_id`.
...
### Why does SecureVu keep creating new events for my parked car?

Stationary tracking is designed to _prevent_ this — a parked car should stay one tracked object and not generate new events. If you're getting repeated events for the same car, it's likely that SecureVu is losing the tracked object and re-detecting it as a new one.
...
- **Use a more accurate model** — switching from `mobiledet` to `yolov9`, or stepping up to a larger variant like `yolov9-s` over `yolov9-t`, can help (at the cost of inference time, and still on the COCO dataset). The biggest gains usually come from fine-tuning a model on images from your own cameras so it learns your specific scene. [SecureVu+](/plus/first_model) is a paid option that does this - models are trained on security-camera footage and can be fine-tuned on images you submit from your own setup.
