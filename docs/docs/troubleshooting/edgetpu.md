...
Using `lsusb` or checking the hardware page in HA OS will show as `1a6e:089a Global Unichip Corp.` until SecureVu runs an inferance using the coral. So don't worry about the identification until after SecureVu has attempted to detect the coral.

:::

If the coral does not initialize then SecureVu can not interface with it. Some common reasons for the USB based Coral not initializing are:
...
- When running SecureVu in a VM, Proxmox lxc, etc. you must ensure both device IDs are mapped.
- When running through the Home Assistant OS you may need to run the Full Access variant of the SecureVu App with the _Protection mode_ switch disabled so that the coral can be accessed.
...
7. Start the securevu container. Everything should work now!

### QNAP NAS

QNAP NAS devices, such as the TS-253A, may use connected Coral TPU devices if [QuMagie](https://www.qnap.com/en/software/qumagie) is installed along with its QNAP AI Core extension. If any of the features—`facial recognition`, `object recognition`, or `similar photo recognition`—are enabled, Container Station applications such as `SecureVu` or `CodeProject.AI Server` will be unable to initialize the TPU device in use.
...
3. Manually start the QNAP AI Core extension after SecureVu has fully started (not recommended).
...
2. Running SecureVu in a VM may cause communication with the device to be lost and need to be reset.
