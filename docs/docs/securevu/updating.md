...
The current stable version of SecureVu is **0.18.0**. The release notes and any breaking changes for this version can be found on the [SecureVu GitHub releases page](https://github.com/SecureCorp-Mexico/SecureVu2/releases/tag/v0.18.0).
...
2. **Update and Pull the Latest Image**:
   - If using Docker Compose:
     - Edit your `docker-compose.yml` file to specify the desired version tag (e.g., `0.18.0` instead of `0.17.1`). For example:
       ...
       services:
         securevu:
           image: ghcr.io/securecorp-mexico/securevu2:0.18.0
       ```
     - Then pull the image:
       ```bash
       docker pull ghcr.io/securecorp-mexico/securevu2:0.18.0
       ```
...
3. Revert to the previous image version:
   - For Docker: Specify an older tag (e.g., `ghcr.io/securecorp-mexico/securevu2:0.17.1`) in your `docker run` command.
