...
The current stable version of SecureVu is **0.17.2**. The release notes and any breaking changes for this version can be found on the [SecureVu GitHub releases page](https://github.com/SecureCorp-Mexico/SecureVu2/releases/tag/v0.17.2).
...
2. **Update and Pull the Latest Image**:
   - If using Docker Compose:
     - Edit your `docker-compose.yml` file to specify the desired version tag (e.g., `0.17.2` instead of `0.16.4`). For example:
       ...
       services:
         securevu:
           image: ghcr.io/securecorp-mexico/securevu2:0.17.2
       ```
     - Then pull the image:
       ```bash
       docker pull ghcr.io/securecorp-mexico/securevu2:0.17.2
       ```
