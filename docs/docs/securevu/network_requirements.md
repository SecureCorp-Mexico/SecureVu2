...
SecureVu is designed to run locally and does not require a persistent internet connection for core functionality. However, certain features need internet access for initial setup or ongoing operation. This page describes what connects to the internet, when, and how to control it.

## How SecureVu Uses the Internet

SecureVu's internet usage falls into three categories:
...
2. **Optional cloud services**: Features like SecureVu+ and Generative AI connect to external APIs only when explicitly configured.
...
After initial setup, SecureVu can run fully offline as long as all required models have been downloaded and no cloud-dependent features are enabled.
...
The MobileNetV2 base weights are the one exception to the `/config/model_cache/` rule. They are also the only entry that is not downloaded when the feature is enabled: SecureVu fetches them when a training run actually starts.
...
If you have already downloaded all required models and want to prevent SecureVu from attempting any outbound connections to HuggingFace or the Transformers library, set the following environment variables on your SecureVu container:
...
If your SecureVu instance has restricted internet access, you can point model downloads at internal mirrors using environment variables:
...
When a SecureVu+ API key is configured, SecureVu communicates with `https://api.secure.vu` to download models, upload snapshots for training, submit annotations, and report false positives. Remove the API key to disable all SecureVu+ network activity.

See [SecureVu+](/integrations/plus) for details.

### Generative AI

When a Generative AI provider is configured, SecureVu sends images and prompts to the configured provider for event descriptions, chat, and camera monitoring. Available providers:
...
SecureVu checks GitHub for the latest release version on startup by querying `https://api.github.com`. This can be disabled:
...
When [notifications](/configuration/notifications) are enabled and users have registered for push notifications in the web UI, SecureVu sends push messages through the browser vendor's push service (e.g., Google FCM, Mozilla autopush). This requires internet access from the SecureVu server to these push endpoints.

### MQTT

If an [MQTT broker](/integrations/mqtt) is configured, SecureVu maintains a connection to the broker's host and port. This is typically a local network connection, but will require internet if you use a cloud-hosted MQTT broker.

### DeepStack / CodeProject.AI

When using the [DeepStack detector plugin](/configuration/object_detectors), SecureVu sends images to the configured API endpoint for inference. This is typically local but depends on where the service is hosted.

## WebRTC (STUN)

For [WebRTC live streaming](/configuration/live), SecureVu uses STUN for NAT traversal:
...
## Running SecureVu Offline

To run SecureVu in an air-gapped or offline environment:

1. **Pre-download models**: Start SecureVu with internet access once with all desired features enabled. Models will be cached in `/config/model_cache/`.
...
5. **Avoid cloud features**: Do not configure SecureVu+, Generative AI providers that require internet, or cloud MQTT brokers.
...
After these steps, SecureVu will operate with no outbound internet connections.
...
Copy the file into your SecureVu config volume as `/config/model_cache/MobileNet/mobilenet_v2_weights.h5`, keeping that exact filename, then set the environment variable `TF_KERAS_MOBILENET_V2_WEIGHTS_URL` in your Docker compose file to the URL above and restart SecureVu.

The variable must be set even though the URL is never contacted. If it is unset, SecureVu ignores the copied file and asks Keras to download the weights instead.
