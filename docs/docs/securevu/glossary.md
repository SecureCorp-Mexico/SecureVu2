...
The glossary explains terms commonly used in SecureVu's documentation.
...
Something SecureVu can detect and follow in a camera frame, identified by its [label](#label) (for example a person or a car). The object types SecureVu watches for are set in the `objects` configuration. Once an object is detected and followed across frames it becomes a [tracked object](#tracked-object-event-in-previous-versions), which may also carry a [sub label](#sub-label) and [attributes](#attribute). [See the available objects docs for more info](/configuration/objects)
...
A user-defined area of interest within the camera frame. Zones can be used for notifications and to limit where SecureVu creates a [review item](#review-item). [See the zone docs for more info](/configuration/zones)
