...
function buildImage(device: DeviceConfig): string[] {
  const tag = device.imageTagSuffix
    ...
    : device.imageTag;
  return [`    image: ghcr.io/securecorp-mexico/securevu2:${tag}`];
}
...
function buildVolumes(
  ...
  return [
    ...
    `      - ${mediaPath}:/media/securevu # Recording storage directory`,
...
function buildEnvironment(
  ...
  if (rtspPassword) {
    lines.push(
      `      SECUREVU_RTSP_PASSWORD: "${rtspPassword}" # RTSP password — change to your own`
    );
  }
