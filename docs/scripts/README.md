...
Automatically generates "SecureVu UI" tab content for documentation files based on the YAML config examples already in the docs.
...
Run from the repository root. The script imports SecureVu's Python config models directly, so the `securevu` package must be importable:

```bash
# From repo root -- no extra install needed if your environment can import securevu
...
|--------|------|------------------|
| Pydantic models | `securevu/config/` | Field names, types, defaults, nesting |
