#!/usr/bin/env python3
"""Generate SecureVu UI tab content for documentation files.

This script reads YAML code blocks from documentation markdown files and
generates corresponding "SecureVu UI" tab instructions based on:
...
# Ensure securevu package is importable
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate SecureVu UI tab content for documentation files"
    )
    parser.add_argument(
        "paths",
        nargs="+",
    ...
    if outdir is not None:
        if str(outdir) == "auto":
            outdir = Path(tempfile.mkdtemp(prefix="securevu-ui-tabs-"))
            created_tmpdir = True
