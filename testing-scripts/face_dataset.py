...
Standalone replica of SecureVu's ArcFace pipeline (see
securevu/data_processing/common/face/model.py and
securevu/embeddings/onnx/face_embedding.py) for analyzing a face collection
...
def _process_image_securevu(image: np.ndarray) -> Image.Image:
...
def arcface_preprocess(image_bgr: np.ndarray) -> np.ndarray:
    ...
    """
    pil = _process_image_securevu(_bgr_to_rgb(image_bgr))
...
def vector_outlier_test(
    ...
    securevu/data_processing/common/face/model.py uses T=0.30. This test
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Analyze a face recognition collection outside SecureVu.",
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.15,
        help="trim_mean proportion (SecureVu uses 0.15)",
    )
    ap.add_argument(
        "--vector-outlier",
        action="store_true",
