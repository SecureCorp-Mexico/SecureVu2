#!/usr/bin/env python3
"""Analyze keyframe and timestamp structure of SecureVu recording segments.

This is a diagnostic tool for investigating seek precision / GOP behavior on
recorded segments. It does not modify anything.

ffprobe is only available inside the SecureVu container, at
...
Typical use:
    # Inside the SecureVu container (or wherever recordings are mounted)
    ...
    python3 analyze_recording_keyframes.py <camera> --recordings-dir /media/securevu/recordings
...
def resolve_ffprobe_path(override: str | None) -> str:
    ...
    Inside the SecureVu container, ffprobe lives at
    ...
    if version:
        ...
        if Path(path).is_file():
            return path
    # Fall back to scanning the SecureVu ffmpeg install root.
...
def find_segments_near_timestamp(
    ...
    # should contain the timestamp (SecureVu catalogs segments by filename).
    target_idx = -1
...
def filename_to_timestamp(segment: Path) -> float | None:
    """Parse the wall-clock time from SecureVu's segment path layout."""
    try:
...
def analyze(ffprobe: str, segment: Path, highlight: bool = False) -> None:
    ...
    # (Filename time is what SecureVu uses as recording.start_time in the DB.)
    if filename_ts is not None and first_pts is not None:
        print(
            f"  Notes: first packet PTS is {first_pts:.3f}s into the file; "
            f"SecureVu treats filename time as PTS=0 for seek math."
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--recordings-dir",
        default="/media/securevu/recordings",
        help="Path to the recordings directory (default: /media/securevu/recordings)",
    )
    parser.add_argument(
        "--ffprobe",
        default=None,
        help=(
            "Full path to the ffprobe binary. Defaults to the SecureVu-bundled "
