...
Standalone replica of SecureVu's custom object classification inference pipeline
(see securevu/data_processing/real_time/custom_classification.py and
securevu/util/classification.py) for analyzing a training dataset outside the
...
Recommended workflow when troubleshooting misclassifications:
  ...
  4. Look for cross-class duplicates from the same SecureVu event. If the
...
The dataset must be the same layout SecureVu trains from:
...
Command-line examples (mirror the workflow steps above):
  ...
  restored. Change `threshold = 80` to 64 (looser) or 100 (stricter):
    ...
    dataset = Path("/media/securevu/clips/<model_name>/dataset")
    quarantine = Path("/media/securevu/clips/<model_name>/quarantine_small")
    threshold = 80
    moved = 0
  ...
  TIMESTAMP with the prefix you saw in worst-offenders, e.g. "1772052999"):
    ...
    cd "/media/securevu/clips/<model_name>/dataset"
...
Full flag reference:
    python3 object_dataset.py \\
        --name <model_name> \\
        [--clips-dir /media/securevu/clips] \\
...
def preprocess_for_inference(image_bgr: np.ndarray) -> np.ndarray:
    ...
    SecureVu decodes the camera frame YUV->RGB, crops, then cv2.resize to
...
def classify_image(
    ...
    the exact `probs = res / res.sum(axis=0)` renormalization SecureVu uses
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description=(
            "Analyze a SecureVu object-classification training dataset against its "
    ...
    ap.add_argument(
        "--clips-dir",
        default="/media/securevu/clips",
        help="SecureVu clips directory; dataset is read from <clips>/<name>/dataset",
    )
    ap.add_argument(
        "--model-cache",
        default="/config/model_cache",
        help="SecureVu model_cache; model is read from <cache>/<name>/model.tflite",
    )
    ap.add_argument(
        "--threshold",
        type=float,
