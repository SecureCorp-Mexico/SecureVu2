...
class TensorRtDetector(DetectionApi):
    ...
    def __init__(self, detector_config: TensorRTDetectorConfig):
        if platform.machine() == "x86_64":
            logger.error(
                "TensorRT detector is no longer supported on amd64 system. Please use ONNX detector instead, see https://docs.secure.vu/configuration/object_detectors#onnx for more information."
            )
            raise ImportError(
                "TensorRT detector is no longer supported on amd64 system. Please use ONNX detector instead, see https://docs.secure.vu/configuration/object_detectors#onnx for more information."
            )
