...
from securevu.util.file import FileLock
from securevu.util.model import xyxy_to_xywh_for_nms
...
class MemryXDetector(DetectionApi):
    ...
    def process_yolo(self, class_id, conf, pos):
        ...
        returns an array that's easily passable back to SecureVu.
        """
        return [
            class_id,  # class ID
    ...
    def post_process_yolo_optimized(self, outputs):
        ...
        if len(indices) > 0:
            ...
            # Convert to SecureVu format: [class_id, confidence, y_min, x_min, y_max, x_max] (normalized)
            for i, idx in enumerate(indices):
                ...
                final_detections[i] = [
                    class_id,
                    confidence,
                    y_min_norm,  # SecureVu expects y_min first
                    x_min_norm,
                    y_max_norm,
                    x_max_norm,
                ]
