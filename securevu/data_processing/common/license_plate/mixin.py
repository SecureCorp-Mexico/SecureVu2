...
from securevu.config.classification import LicensePlateRecognitionConfig
from securevu.const import CLIPS_DIR, MODEL_CACHE_DIR
from securevu.data_processing.common.license_plate.model import LicensePlateModelRunner
from securevu.embeddings.onnx.lpr_embedding import LPR_EMBEDDING_SIZE
from securevu.types import TrackedObjectUpdateTypesEnum
from securevu.util.builtin import EventsPerSecond, InferenceSpeed
from securevu.util.image import area
...
class LicensePlateProcessingMixin:
    ...
    def _detect_license_plate(
        ...
        """
        Use a lightweight YOLOv9 model to detect license plates for users without SecureVu+
    ...
    def lpr_process(
        ...
        # dedicated LPR cam without securevu+
        if dedicated_lpr:
            id = "dedicated-lpr"
            ...
            # don't run for non car/motorcycle or non license plate (dedicated lpr with securevu+) objects
            if (
            ...
            if "license_plate" not in self.config.cameras[camera].objects.track:
                ...
                # don't run for object without attributes if this isn't dedicated lpr with securevu+
                if (
                ...
                # we are using dedicated lpr with securevu+
        ...
        # save the best snapshot for dedicated lpr cams not using securevu+
        if (
            dedicated_lpr
            and "license_plate" not in self.config.cameras[camera].objects.track
        ):
