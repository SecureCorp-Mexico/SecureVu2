...
from securevu.app import SecureVuApp
from securevu.config import SecureVuConfig
from securevu.log import setup_logging
...
def main() -> None:
    ...
    threading.current_thread().name = "securevu"
    stop_event = mp.Event()
    ...
    try:
        config = SecureVuConfig.load(install=True)
    except ValidationError as e:
        print("*************************************************************")
        print("*************************************************************")
        print("***    Your config file is not valid!                     ***")
        print("***    Please check the docs at                           ***")
        print("***    https://docs.secure.vu/configuration/          ***")
        print("*************************************************************")
        print("*************************************************************")
        ...
        # attempt to start SecureVu in recovery mode
        try:
            config = SecureVuConfig.load(install=True, safe_load=True)
            print("Starting SecureVu in safe mode.")
        except ValidationError:
            print("Unable to start SecureVu in safe mode.")
            sys.exit(1)
    ...
    SecureVuApp(config, manager, stop_event).start()
...
if __name__ == "__main__":
    mp.set_forkserver_preload(
        [
            ...
            "sqlite3",
            # Third-party libraries commonly used in SecureVu
            "numpy",
            "cv2",
            "peewee",
            "zmq",
            "ruamel.yaml",
            # SecureVu core modules
            "securevu.camera.maintainer",
        ]
    )
