...
from securevu.util.ffmpeg import inject_progress_flags
...
def _make_exporter(
    ...
    effects (which create directories and require a full SecureVuConfig)."""
...
class TestFfmpegProgressParsing(unittest.TestCase):
    ...
    def _run_with_stderr(
        ...
        with patch("securevu.util.ffmpeg.sp.Popen", return_value=fake_proc):
...
class TestBroadcastAggregation(unittest.TestCase):
    ...
    def test_throttle_allows_broadcast_after_interval(self) -> None:
        ...
        with patch("securevu.jobs.export.time.monotonic") as mock_mono:
...
class TestSaveThumbnailFromPreviewFrames(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.tmp_root = tempfile.mkdtemp(prefix="securevu_thumb_test_")
    ...
    def test_short_export_falls_back_to_prior_preview_frame(self) -> None:
        ...
        with (
            patch(
                "securevu.record.export.CACHE_DIR", os.path.join(self.tmp_root, "cache")
            ),
            patch(
                "securevu.record.export.CLIPS_DIR", os.path.join(self.tmp_root, "clips")
            ),
        ):
...
class TestSchedulesCleanup(unittest.TestCase):
    def test_schedule_job_cleanup_removes_after_delay(self) -> None:
        ...
        with patch("securevu.jobs.export.threading.Timer") as mock_timer:
...
class TestChapterMetadataInProgressReview(unittest.TestCase):
    ...
    def test_in_progress_review_does_not_crash_and_clamps_to_last_recording(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "securevu.record.export.ReviewSegment.select",
