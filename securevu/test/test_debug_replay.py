...
Startup orchestration lives in ``securevu.jobs.debug_replay`` (covered by
...
class TestDebugReplayManagerStop(unittest.TestCase):
    def setUp(self) -> None:
        ...
        patcher = patch("securevu.debug_replay.JobStatePublisher")
        patcher.start()
    ...
    def test_stop_when_inactive_is_a_noop(self) -> None:
        ...
        manager = DebugReplayManager()
        securevu_config = MagicMock()
        securevu_config.cameras = {}
        publisher = MagicMock()

        # Should not raise; should not publish any events.
        manager.stop(securevu_config=securevu_config, config_publisher=publisher)

        publisher.publish_update.assert_not_called()

    def test_stop_publishes_remove_when_camera_was_published(self) -> None:
        from securevu.config.camera.updater import CameraConfigUpdateEnum
        from securevu.debug_replay import DebugReplayManager
        ...
        securevu_config = MagicMock()
        securevu_config.cameras = {"_replay_front": camera_config}
        publisher = MagicMock()

        with (
            patch.object(manager, "_cleanup_db"),
            patch.object(manager, "_cleanup_files"),
            patch("securevu.debug_replay.cancel_debug_replay_job", return_value=False),
        ):
            manager.stop(securevu_config=securevu_config, config_publisher=publisher)
    ...
    def test_stop_skips_remove_publish_when_camera_not_in_config(self) -> None:
        ...
        securevu_config = MagicMock()
        securevu_config.cameras = {}  # _replay_front not present
        publisher = MagicMock()

        with (
            patch.object(manager, "_cleanup_db"),
            patch.object(manager, "_cleanup_files"),
            patch("securevu.debug_replay.cancel_debug_replay_job", return_value=True),
        ):
            manager.stop(securevu_config=securevu_config, config_publisher=publisher)
    ...
    def test_stop_calls_cancel_debug_replay_job(self) -> None:
        ...
        with (
            ...
            patch.object(manager, "_cleanup_files"),
            patch(
                "securevu.debug_replay.cancel_debug_replay_job",
                return_value=True,
            ) as mock_cancel,
        ):
            manager.stop(securevu_config=securevu_config, config_publisher=publisher)
...
class TestDebugReplayManagerPublishCamera(unittest.TestCase):
    def test_publish_camera_invokes_publisher_with_add_topic(self) -> None:
        ...
        securevu_config = MagicMock()
        securevu_config.cameras = {"front": source_config}
        publisher = MagicMock()
        ...
        with (
            ...
            patch("securevu.debug_replay.find_config_file", return_value="/cfg.yml"),
            patch("securevu.debug_replay.YAML") as yaml_cls,
            patch("securevu.debug_replay.SecureVuConfig.parse_object") as parse_object,
            ...
            manager.publish_camera(
                ...
                securevu_config=securevu_config,
                config_publisher=publisher,
            )

        # Camera registered into the live config dict
        self.assertIn("_replay_front", securevu_config.cameras)
    ...
    def test_publish_camera_wraps_parse_failure_in_runtime_error(self) -> None:
        ...
        manager = DebugReplayManager()
        securevu_config = MagicMock()
        securevu_config.cameras = {"front": MagicMock()}
        publisher = MagicMock()
        ...
        with (
            ...
            patch("securevu.debug_replay.YAML") as yaml_cls,
            patch(
                "securevu.debug_replay.SecureVuConfig.parse_object",
