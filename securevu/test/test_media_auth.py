"""Unit tests for `securevu.api.media_auth`.
...
from securevu.models import Event, Export, Recordings, ReviewSegment
...
class TestResolveMediaUri(unittest.TestCase):
    def setUp(self):
        self.config = SecureVuConfig(**_CONFIG)
    ...
    def test_clip_review_thumbs(self):
        # Format: /clips/review/thumb-{camera}-{review_id}.webp (securevu/review/maintainer.py).
        self._assert(
...
class TestExportResolution(unittest.TestCase):
    ...
    def _insert_export(self, export_id, camera, filename):
        Export.insert(
            ...
            video_path=f"/media/securevu/exports/{filename}",
            thumb_path=f"/media/securevu/exports/{filename}.jpg",
            in_progress=False,
        ).execute()
    ...
    def test_export_anchored_match_not_endswith(self):
        ...
        # /media/securevu/exports/back_door_clip.mp4 just because the suffix matches.
