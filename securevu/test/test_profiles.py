...
from securevu.config.camera.profile import CameraProfileConfig
from securevu.config.profile import ProfileDefinitionConfig
from securevu.config.profile_manager import PERSISTENCE_FILE, ProfileManager
...
class TestCameraProfileConfig(unittest.TestCase):
    ...
    def test_invalid_section_key_rejected(self):
        """Unknown section keys are rejected (extra=forbid from SecureVuBaseModel)."""
        from pydantic import ValidationError
...
class TestProfileInConfig(unittest.TestCase):
    ...
    def test_profiles_parse(self):
        ...
        config = SecureVuConfig(**self.base_config)
...
class TestProfileManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = SecureVuConfig(**self.config_data)
    ...
    def test_activate_profile_adds_zone(self, mock_persist):
        ...
        from securevu.config.camera.zone import ZoneConfig
    ...
    def test_profile_change_republishes_switch_states(self, mock_persist):
        ...
        config (and SecureVu's behavior) but left the retained MQTT state
    ...
    def test_update_config_preserves_runtime_state_with_active_profile(
        ...
        new_config = SecureVuConfig(**self.config_data)
