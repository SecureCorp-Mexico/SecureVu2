...
response" so the two conversion layers are exercised without any network:
  ...
  1. SecureVu (OpenAI-style) messages -> provider-native request format
  2. provider-native response -> SecureVu ``("kind", value)`` stream events
...
from securevu.config import GenAIConfig, GenAIProviderEnum
...
class TestOllamaProvider(unittest.TestCase):
    ...
    def _run_with_response(self, client, response, messages):
        ...
        with patch(
            "securevu.genai.plugins.ollama.OllamaAsyncClient",
    ...
    def test_normalize_multimodal_content(self):
        from securevu.genai.plugins.ollama import _normalize_multimodal_content
...
class TestLlamaCppProvider(unittest.TestCase):
    ...
    def _run_with_lines(self, client, lines, messages):
        with patch(
            "securevu.genai.plugins.llama_cpp.httpx.AsyncClient",
