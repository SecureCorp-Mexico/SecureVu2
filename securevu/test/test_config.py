...
class TestConfig(unittest.TestCase):
    ...
    def test_default_audio_filters(self):
        ...
        securevu_config = SecureVuConfig(**config)
        assert set(securevu_config.cameras["back"].audio.filters.keys()) == {
            "speech",
            "yell",
        }
    ...
    def test_override_audio_filters(self):
        ...
        securevu_config = SecureVuConfig(**config)
        assert "speech" in securevu_config.cameras["back"].audio.filters
        assert securevu_config.cameras["back"].audio.filters["speech"].threshold == 0.9
        assert "yell" in securevu_config.cameras["back"].audio.filters
        assert "babbling" not in securevu_config.cameras["back"].audio.filters
    ...
    def test_unsupported_tracked_object_pruned_from_track_and_filters(self):
        ...
        securevu_config = SecureVuConfig(**config)
        objects = securevu_config.cameras["back"].objects
    ...
    def test_motion_mask_relative_matches_explicit(self):
        ...
        assert np.array_equal(
            securevu_config.cameras["explicit"].motion.rasterized_mask,
            securevu_config.cameras["relative"].motion.rasterized_mask,
        )
    ...
    def test_default_snapshots(self):
        ...
        assert securevu_config.cameras["back"].snapshots.quality == 60
...
class TestAttributeFilterDefaults(unittest.TestCase):
    ...
    def _build_config(self, object_filters: dict | None = None) -> SecureVuConfig:
        config = deep_merge({}, self.minimal)
        if object_filters is not None:
            config.setdefault("objects", {})["filters"] = object_filters
        return SecureVuConfig(**config)
