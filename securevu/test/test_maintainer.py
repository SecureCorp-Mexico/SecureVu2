...
sys.modules["securevu.comms.inter_process"] = MagicMock()
sys.modules["securevu.comms.detections_updater"] = MagicMock()
sys.modules["securevu.comms.recordings_updater"] = MagicMock()
sys.modules["securevu.config.camera.updater"] = MagicMock()

# Now import the class under test
from securevu.config import SecureVuConfig  # noqa: E402
from securevu.record.maintainer import RecordingMaintainer  # noqa: E402


class TestMaintainer(unittest.IsolatedAsyncioTestCase):
    async def test_move_files_survives_bad_filename(self):
        config = MagicMock(spec=SecureVuConfig)
        config.cameras = {}
        ...
        with patch("os.listdir", return_value=files):
            with patch("os.path.isfile", return_value=True):
                with patch(
                    "securevu.record.maintainer.psutil.process_iter", return_value=[]
                ):
                    with patch("securevu.record.maintainer.logger.warning") as warn:
