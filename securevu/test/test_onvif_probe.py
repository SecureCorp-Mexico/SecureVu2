...
from securevu.api.camera import _build_digest_transport, _connect_onvif_camera
...
class TestConnectOnvifCamera(unittest.IsolatedAsyncioTestCase):
    async def test_password_digest_succeeds_first(self):
        ...
        with patch("securevu.api.camera.ONVIFCamera", return_value=camera) as mock_cls:
    ...
    async def test_falls_back_to_password_text(self):
        ...
        with patch(
            "securevu.api.camera.ONVIFCamera",
    ...
    async def test_transport_error_is_not_retried(self):
        ...
        with patch("securevu.api.camera.ONVIFCamera", side_effect=[camera]) as mock_cls:
    ...
    async def test_digest_auth_replaces_service_transports(self):
        ...
        with (
            patch("securevu.api.camera.ONVIFCamera", return_value=camera),
            patch(
                "securevu.api.camera._build_digest_transport",
    ...
    async def test_basic_auth_does_not_replace_transports(self):
        ...
        with (
            patch("securevu.api.camera.ONVIFCamera", return_value=camera),
            patch("securevu.api.camera._build_digest_transport") as mock_transport,
        ):
