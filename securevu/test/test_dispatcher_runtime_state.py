...
def _build_dispatcher(cameras: dict[str, MagicMock]) -> Dispatcher:
    ...
    with (
        patch("securevu.comms.dispatcher.CameraActivityManager"),
        patch("securevu.comms.dispatcher.AudioActivityManager"),
    ):
...
class TestHandlersPersistViaSet(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        with open(self.config_path, "w") as f:
            f.write("")
        self._patcher = patch(
            "securevu.comms.runtime_state.find_config_file",
...
class TestStartupAppliesConfigLayersBeforeWorkersStart(unittest.TestCase):
    ...
    def _start_call_order(self) -> list[str]:
        """Return the names SecureVuApp.start() calls, in order."""
        app = MagicMock()

        with (
            patch("securevu.app.set_file_limit"),
            patch("securevu.app.cleanup_replay_cameras"),
            patch("securevu.app.reap_stale_exports"),
            patch("securevu.app.create_fastapi_app"),
            patch("securevu.app.uvicorn"),
        ):
            SecureVuApp.start(app)
