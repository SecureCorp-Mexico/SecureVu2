...
class TestCameraAccessEventReview(BaseTestHttp):
    ...
    def test_event_single_access(self):
        ...
        with patch("securevu.api.event.require_camera_access", mock_require_allowed):
            with AuthTestClient(self.app) as client:
        ...
        with patch("securevu.api.event.require_camera_access", mock_require_disallowed):
            with AuthTestClient(self.app) as client:
                resp = client.get("/events/event1")
    ...
    def test_review_single_access(self):
        ...
        with patch("securevu.api.review.require_camera_access", mock_require_allowed):
            with AuthTestClient(self.app) as client:
...
class TestGo2rtcStreamAccess(BaseTestHttp):
    ...
    def test_add_stream_allows_restricted_source_when_override_set(self):
        ...
        with patch.dict(os.environ, {"GO2RTC_ALLOW_ARBITRARY_EXEC": "true"}):
            with patch(
                "securevu.api.camera.requests.put", return_value=mock_response
            ) as mock_put:
                with AuthTestClient(app) as client:
