...
class TestConfigSetWildcardPropagation(BaseTestHttp):
    ...
    def _create_app_with_publisher(self):
        ...
        app = create_fastapi_app(
            SecureVuConfig(**self.minimal_config),
            self.db,
            None,
            None,
            None,
    ...
    @patch("securevu.api.app.find_config_file")
    def test_runtime_disabled_camera_survives_unrelated_save(self, mock_find_config):
        ...
        try:
            ...
            with AuthTestClient(app) as client:
                ...
                self.assertFalse(app.securevu_config.cameras["front_door"].enabled)
                self.assertIs(dispatcher.config, app.securevu_config)
    ...
    def test_save_updates_the_config_holder(self, mock_find_config):
        ...
        SecureVuApp reads the holder when the watchdog rebuilds a crashed
        process; if the save leaves it on the boot config, that process comes
        back having lost every change made since SecureVu started.
        """
        from fastapi import Request
        ...
        mock_publisher = Mock(spec=CameraConfigUpdatePublisher)
        mock_publisher.publisher = MagicMock()
        boot_config = SecureVuConfig(**self.minimal_config)
        ...
        try:
            ...
            self.assertIs(holder.config, app.securevu_config)
