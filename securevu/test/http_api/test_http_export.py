...
from securevu.test.http_api.base_http_test import AuthTestClient, BaseTestHttp
...
class TestHttpExport(BaseTestHttp):
    ...
    def test_create_export_case_uses_wall_clock_time(self):
        with patch("securevu.api.export.time.time", return_value=1234.5):
    ...
    def test_update_export_case_refreshes_updated_at(self):
        ...
        with patch("securevu.api.export.time.time", return_value=2222.0):
            with AuthTestClient(self.app) as client:
    ...
    def test_delete_export_case_delete_exports_cancels_queued_jobs(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("securevu.jobs.export._job_manager", None),
                patch(
                    "securevu.jobs.export.ExportJobManager.ensure_started",
                    autospec=True,
                    return_value=None,
                ),
            ):
                start_export_job(
                    self.app.securevu_config,
                    ExportJob(
                        id="queued_case_job",
                        camera="front_door",
                ...
                manager = get_export_job_manager(self.app.securevu_config)
    ...
    def test_batch_export_creates_case_and_reports_partial_success(self):
        ...
        with patch(
            "securevu.api.export.start_export_job",
    ...
    def test_single_export_returns_503_when_queue_full(self):
        ...
        from securevu.jobs.export import ExportQueueFullError
    ...
    def test_batch_export_returns_503_when_queue_cannot_fit_batch(self):
        ...
        with patch(
            "securevu.api.export.available_export_queue_slots",
            return_value=1,
        ):
            with patch(
                "securevu.api.export.start_export_job",
    ...
    def test_get_active_export_jobs_returns_queue_state(self):
        ...
        with patch(
            "securevu.api.export.list_active_export_jobs",
    ...
    def test_batch_export_camera_access_denied_fails_closed(self):
        ...
        from securevu.api.auth import get_allowed_cameras_for_filter
    ...
    def test_bulk_delete_exports_in_progress(self):
        ...
        Export.create(
            ...
            name="export_1",
            date=100,
            video_path=f"{os.environ.get('EXPORT_DIR', '/media/securevu/exports')}/exp1.mp4",
            thumb_path="/tmp/exp1.jpg",
            in_progress=True,
        )

        with patch(
            "securevu.api.export._get_files_in_use",
