...
from securevu.api.auth import get_allowed_cameras_for_filter, get_current_user
from securevu.comms.event_metadata_updater import EventMetadataPublisher
from securevu.models import Event, Recordings, ReviewSegment, Timeline
from securevu.stats.emitter import StatsEmitter
from securevu.test.http_api.base_http_test import AuthTestClient, BaseTestHttp, Request
from securevu.test.test_storage import _insert_mock_event
...
class TestHttpApp(BaseTestHttp):
    ...
    def test_events_search_match_multilingual_attribute(self):
        ...
        self.app.securevu_config.semantic_search.enabled = True
    ...
    def test_get_metrics(self):
        ...
        assert "# TYPE securevu_detection_total_fps gauge" in event.text
        assert "securevu_detection_total_fps 13.7" in event.text
        assert (
            "# HELP securevu_camera_events_total Count of camera events since exporter started"
            in event.text
        )
        assert "# TYPE securevu_camera_events_total counter" in event.text
        assert (
            'securevu_camera_events_total{camera="front_door",label="Mock"} 3.0'
            in event.text
        )
        assert (
            'securevu_camera_events_total{camera="front_door",label="inside"} 2.0'
            in event.text
        )
        assert (
            'securevu_camera_events_total{camera="front_door",label="outside"} 2.0'
            in event.text
        )
        assert (
