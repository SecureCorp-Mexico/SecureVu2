...
from securevu.api.auth import hash_password
from securevu.api.fastapi_app import create_fastapi_app
from securevu.camera import CameraMetrics, PTZMetrics
from securevu.camera.maintainer import CameraMaintainer
from securevu.comms.base_communicator import Communicator
from securevu.comms.dispatcher import Dispatcher
from securevu.comms.event_metadata_updater import EventMetadataPublisher
from securevu.comms.inter_process import InterProcessCommunicator
from securevu.comms.mqtt import MqttClient
from securevu.comms.object_detector_signaler import DetectorProxy
from securevu.comms.webpush import WebPushClient
from securevu.comms.ws import WebSocketClient
from securevu.comms.zmq_proxy import ZmqProxy
...
from securevu.db.sqlitevecq import SqliteVecQueueDatabase
from securevu.debug_replay import (
    DebugReplayManager,
    cleanup_replay_cameras,
)
from securevu.embeddings import EmbeddingProcess, EmbeddingsContext
from securevu.events.audio import AudioProcessor
from securevu.events.cleanup import EventCleanup
from securevu.events.maintainer import EventProcessor
from securevu.jobs.export import reap_stale_exports
from securevu.jobs.motion_search import stop_all_motion_search_jobs
from securevu.log import _stop_logging
from securevu.models import (
    Event,
    Export,
    Previews,
    Recordings,
...
from securevu.output.output import OutputProcess
from securevu.ptz.autotrack import PtzAutoTrackerThread
from securevu.ptz.onvif import OnvifController
from securevu.record.cleanup import RecordingCleanup
from securevu.record.export import migrate_exports
from securevu.record.record import RecordProcess
from securevu.review.review import ReviewProcess
from securevu.stats.emitter import StatsEmitter
from securevu.stats.util import stats_init
from securevu.storage import StorageMaintainer
from securevu.timeline import TimelineProcessor
from securevu.track.object_processing import TrackedObjectProcessor
from securevu.util.builtin import empty_and_close_queue
from securevu.util.image import UntrackedSharedMemory
from securevu.util.process import SecureVuProcess
from securevu.util.services import set_file_limit
from securevu.version import VERSION
from securevu.watchdog import SecureVuWatchdog
...
class SecureVuApp:
    def __init__(
        self, config: SecureVuConfig, manager: SyncManager, stop_event: MpEvent
    ) -> None:
    ...
    @property
    def config(self) -> SecureVuConfig:
        """The current config, not the one SecureVu booted with.
    ...
    def init_database(self) -> None:
        ...
        if len(router.diff) > 0:
            ...
            shutil.copyfile(
                ...
                self.config.database.path.replace("securevu.db", "backup.db"),
            )
    ...
    def start_watchdog(self) -> None:
        self.securevu_watchdog = SecureVuWatchdog(self.detectors, self.stop_event)

        # (attribute on self, key in self.processes, factory)
        specs: list[tuple[str, str, Callable[[], SecureVuProcess]]] = [
            (
                "embedding_process",
                "embeddings",
        ...
        for attr, key, factory in specs:
            ...
            def on_restart(
                proc: SecureVuProcess, _attr: str = attr, _key: str = key
            ) -> None:
                setattr(self, _attr, proc)
                self.processes[_key] = proc.pid or 0

            self.securevu_watchdog.register(
                key, getattr(self, attr), factory, on_restart
            )

        self.securevu_watchdog.start()
    ...
    def start(self) -> None:
        logger.info(f"Starting SecureVu ({VERSION})")
    ...
    def stop(self) -> None:
        ...
        Path("/dev/shm/.securevu-is-stopping").touch()
