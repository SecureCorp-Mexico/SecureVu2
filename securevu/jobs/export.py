...
from securevu.config.camera.record import ChaptersEnum
from securevu.const import UPDATE_JOB_STATE
from securevu.jobs.job import Job
from securevu.models import Export
from securevu.record.export import PlaybackSourceEnum, RecordingExporter
...
def _get_max_concurrent(config: SecureVuConfig) -> int:
...
def reap_stale_exports() -> None:
    ...
    On SecureVu startup no export job is alive yet, so any in_progress=True
    ...
    active jobs — i.e., at SecureVu startup, before any worker runs.
...
def get_export_job_manager(config: SecureVuConfig) -> ExportJobManager:
...
def start_export_job(config: SecureVuConfig, job: ExportJob) -> str:
    """Queue an export job and return its ID."""
    return get_export_job_manager(config).enqueue(job)


def get_export_job(config: SecureVuConfig, job_id: str) -> ExportJob | None:
    """Get a queued or completed export job by ID."""
    return get_export_job_manager(config).get_job(job_id)


def list_active_export_jobs(config: SecureVuConfig) -> list[ExportJob]:
...
def cancel_queued_export_jobs_for_case(
    config: SecureVuConfig, case_id: str
) -> list[ExportJob]:
...
def available_export_queue_slots(config: SecureVuConfig) -> int:
