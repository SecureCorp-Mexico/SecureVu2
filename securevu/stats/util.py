...
from securevu.const import CACHE_DIR, CLIPS_DIR, RECORD_DIR
...
from securevu.types import StatsTrackingTypes
...
def get_latest_version(config: SecureVuConfig) -> str:
    if not config.telemetry.version_check:
        return "disabled"

    try:
        request = requests.get(
            "https://api.github.com/repos/SecureCorp-Mexico/SecureVu2/releases/latest",
            timeout=10,
        )
...
def stats_init(
    ...
    stats_tracking: StatsTrackingTypes = {
        ...
        "latest_securevu_version": get_latest_version(config),
...
def get_processing_stats(
    config: SecureVuConfig, stats: dict[str, str], hwaccel_errors: list[str]
) -> None:
...
async def set_gpu_stats(
    config: SecureVuConfig, all_stats: dict[str, Any], hwaccel_errors: list[str]
) -> None:
...
async def set_npu_usages(config: SecureVuConfig, all_stats: dict[str, Any]) -> None:
    stats: dict[str, dict] = {}
...
def stats_snapshot(
    ...
    stats["service"] = {
        ...
        "version": VERSION,
        "latest_version": stats_tracking["latest_securevu_version"],
        "storage": {},
