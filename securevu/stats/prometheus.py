...
class CustomCollector:
    ...
    def add_metric_process(
        ...
        try:
            ...
            try:
                # new securevu:0.13.0-beta3 stat 'cmdline'
    ...
    def collect(self):
        ...
        audio_dBFS = GaugeMetricFamily(
            "securevu_audio_dBFS", "Audio dBFS for camera", labels=["camera_name"]
        )
        audio_rms = GaugeMetricFamily(
            "securevu_audio_rms", "Audio RMS for camera", labels=["camera_name"]
        )
        ...
        detection_enabled = GaugeMetricFamily(
            "securevu_detection_enabled",
            "Detection enabled for camera",
            labels=["camera_name"],
        )
        detection_fps = GaugeMetricFamily(
            "securevu_detection_fps",
            "Number of times detection is run per second.",
            labels=["camera_name"],
        )
        process_fps = GaugeMetricFamily(
            "securevu_process_fps",
            "Frames per second being processed by securevu.",
            labels=["camera_name"],
        )
        skipped_fps = GaugeMetricFamily(
            "securevu_skipped_fps",
            "Frames per second skip for processing by securevu.",
            labels=["camera_name"],
        )

        # read camera stats assuming version < securevu:0.13.0-beta3
        cameras = stats
        try:
            # try to read camera stats in case >= securevu:0.13.0-beta3
        ...
        bandwidth_usages = GaugeMetricFamily(
            "securevu_bandwidth_usages_kBps",
        ...
        gpu_usages = GaugeMetricFamily(
            "securevu_gpu_usage_percent", "GPU utilisation %", labels=["gpu_name"]
        )
        gpu_mem_usages = GaugeMetricFamily(
            "securevu_gpu_mem_usage_percent", "GPU memory usage %", labels=["gpu_name"]
        )
        gpu_enc_usages = GaugeMetricFamily(
            "securevu_gpu_encoder_usage_percent",
            "GPU encoder utilisation %",
            labels=["gpu_name"],
        )
        gpu_compute_usages = GaugeMetricFamily(
            "securevu_gpu_compute_usage_percent",
            "GPU compute / encode utilisation %",
            labels=["gpu_name"],
        )
        gpu_dec_usages = GaugeMetricFamily(
            "securevu_gpu_decoder_usage_percent",
        ...
        uptime_seconds = GaugeMetricFamily(
            "securevu_service_uptime_seconds", "Uptime seconds"
        )
        last_updated_timestamp = GaugeMetricFamily(
            "securevu_service_last_updated_timestamp",
        ...
        try:
            ...
            yield InfoMetricFamily(
                "securevu_service", "SecureVu version info", value=info
            )
        ...
        temperatures = GaugeMetricFamily(
            "securevu_device_temperature", "Device Temperature", labels=["device"]
        )
        try:
        ...
        storage_free = GaugeMetricFamily(
            "securevu_storage_free_bytes", "Storage free bytes", labels=["storage"]
        )
        storage_mount_type = InfoMetricFamily(
            "securevu_storage_mount_type",
            "Storage mount type",
            labels=["mount_type", "storage"],
        )
        storage_total = GaugeMetricFamily(
            "securevu_storage_total_bytes", "Storage total bytes", labels=["storage"]
        )
        storage_used = GaugeMetricFamily(
            "securevu_storage_used_bytes", "Storage used bytes", labels=["storage"]
        )
