...
def _collect_zone_names(config: SecureVuConfig) -> set[str]:
...
def _ws_valid_roles(ws: Any, config: SecureVuConfig) -> list[str]:
...
def _ws_is_unrestricted(ws: Any, config: SecureVuConfig) -> bool:
    """True when the connection has unrestricted camera access.

    Mirrors the policy in ``securevu.output.ws_auth``: admin or any role with
...
def _ws_allowed_cameras(ws: Any, config: SecureVuConfig) -> set[str]:
...
def _wrap_envelope(topic: str, inner_payload: Any) -> str:
    ...
    SecureVu's wire format keeps payloads as JSON-encoded strings inside the
...
class WebSocketClient(Communicator):
    """SecureVu wrapper for ws client."""

    def __init__(self, config: SecureVuConfig) -> None:
        self.config = config
