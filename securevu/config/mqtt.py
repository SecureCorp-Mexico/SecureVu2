...
class MqttConfig(SecureVuBaseModel):
    enabled: bool = Field(default=True, title="Enable MQTT Communication.")
    host: EnvString = Field(default="", title="MQTT Host")
    port: int = Field(default=1883, title="MQTT Port")
    topic_prefix: str = Field(default="securevu", title="MQTT Topic Prefix")
    client_id: str = Field(default="securevu", title="MQTT Client ID")
