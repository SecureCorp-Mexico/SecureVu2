...
class HeaderMappingConfig(SecureVuBaseModel):
    user: str = Field(
        default=None,
        title="User header",
    ...
    role_map: dict[str, list[str]] | None = Field(
        ...
        title=("Role mapping"),
        description="Map upstream group values to SecureVu roles (for example map admin groups to the admin role).",
    )


class ProxyConfig(SecureVuBaseModel):
    header_map: HeaderMappingConfig = Field(
        default_factory=HeaderMappingConfig,
        title="Header mapping",
        description="Map incoming proxy headers to SecureVu user and role fields for proxy-based auth.",
    )
