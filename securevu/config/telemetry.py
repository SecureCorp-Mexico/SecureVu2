...
class StatsConfig(SecureVuBaseModel):
...
class TelemetryConfig(SecureVuBaseModel):
    ...
    version_check: bool = Field(
        default=True,
        title="Version check",
        description="Enable an outbound check to detect if a newer SecureVu version is available.",
    )
