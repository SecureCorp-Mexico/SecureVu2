...
class AudioTranscriptionConfig(SecureVuBaseModel):
...
class BirdClassificationConfig(SecureVuBaseModel):
...
class CustomClassificationStateCameraConfig(SecureVuBaseModel):
    crop: list[float, float, float, float] = Field(
        title="Crop of image frame on this camera to run classification on."
    )


class CustomClassificationStateConfig(SecureVuBaseModel):
...
class CustomClassificationObjectConfig(SecureVuBaseModel):
...
class CustomClassificationConfig(SecureVuBaseModel):
...
class ClassificationConfig(SecureVuBaseModel):
...
class SemanticSearchConfig(SecureVuBaseModel):
...
class TriggerConfig(SecureVuBaseModel):
    friendly_name: Optional[str] = Field(
        None, title="Trigger friendly name used in the SecureVu UI."
    )
...
class CameraSemanticSearchConfig(SecureVuBaseModel):
...
class FaceRecognitionConfig(SecureVuBaseModel):
...
class CameraFaceRecognitionConfig(SecureVuBaseModel):
    enabled: bool = Field(default=False, title="Enable face recognition.")
    min_area: int = Field(
        default=750, title="Min area of face box to consider running face recognition."
    )
...
class ReplaceRule(SecureVuBaseModel):
...
class LicensePlateRecognitionConfig(SecureVuBaseModel):
...
class CameraLicensePlateRecognitionConfig(SecureVuBaseModel):
    enabled: bool = Field(default=False, title="Enable license plate recognition.")
...
class CameraAudioTranscriptionConfig(SecureVuBaseModel):
    enabled: bool = Field(default=False, title="Enable audio transcription.")
