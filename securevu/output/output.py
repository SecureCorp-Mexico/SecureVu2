"""Handle outputting raw securevu frames"""
...
from securevu.comms.ws import WebSocket
...
from securevu.output.birdseye import Birdseye
from securevu.output.camera import JsmpegCamera
from securevu.output.preview import PreviewRecorder
from securevu.output.ws_auth import ws_has_camera_access
from securevu.util.image import SharedMemoryFrameManager, get_blank_yuv_frame
...
class OutputProcess(SecureVuProcess):
    def __init__(self, config: SecureVuConfig, stop_event: MpEvent) -> None:
        super().__init__(
            stop_event, PROCESS_PRIORITY_MED, name="securevu.output", daemon=True
        )
        self.config = config
