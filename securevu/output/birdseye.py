...
from securevu.config import BirdseyeModeEnum, FfmpegConfig, SecureVuConfig
from securevu.const import BASE_DIR, BIRDSEYE_PIPE, INSTALL_DIR, UPDATE_BIRDSEYE_LAYOUT
...
class BirdsEyeFrameManager:
    def __init__(
        ...
        # initialize the frame as black and with the SecureVu logo
        ...
        if birdseye_logo is None:
            logo_files = glob.glob(
                os.path.join(INSTALL_DIR, "securevu/images/birdseye.png")
            )
        ...
        if birdseye_logo is not None:
            ...
            logger.warning("Unable to read SecureVu logo")
