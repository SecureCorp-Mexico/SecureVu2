...
from securevu.util.builtin import clean_camera_user_pass, escape_special_characters

logger = logging.getLogger(__name__)


def restart_securevu():
    proc = psutil.Process(1)
    # if this is running via s6, sigterm pid 1
    if proc.name() == "s6-svscan":
        proc.terminate()
    # otherwise, just try and exit securevu
    else:
...
def get_cpu_stats() -> dict[str, dict]:
    ...
    usages["securevu.full_system"] = {
        "cpu": str(system_cpu),
        "mem": str(system_mem.percent),
    }

    keywords = ["ffmpeg", "go2rtc", "securevu.", "python3"]
...
def get_bandwidth_stats(config) -> dict[str, dict]:
    ...
    if p.returncode != 0:
        ...
        for line in lines:
            ...
            try:
                if re.search(
                    r"(^ffmpeg|\/go2rtc|securevu\.detector\.[a-z]+)/([0-9]+)/", stats[0]
                ):
...
def _idle_intel_gpu_stats(
    ...
    """
    from securevu.stats.intel_gpu_info import intel_gpu_name_resolver
...
def process_logs(
    ...
    for raw_line in contents.splitlines():
        ...
        try:
            ...
            # For securevu, remove the date part from message comparison
            if service == "securevu":
