...
from securevu.util.services import get_video_properties
...
def migrate_securevu_config(config_file: str):
    """handle migrating the securevu config."""
    logger.info("Checking if securevu config needs migration...")
    ...
    if previous_version == CURRENT_CONFIG_VERSION:
        logger.info("securevu config does not need migration...")
        return
    ...
    if previous_version < "0.14":
        logger.info(f"Migrating securevu config from {previous_version} to 0.14...")
    ...
    if previous_version < "0.15-0":
        logger.info(f"Migrating securevu config from {previous_version} to 0.15-0...")
    ...
    if previous_version < "0.15-1":
        logger.info(f"Migrating securevu config from {previous_version} to 0.15-1...")
    ...
    if previous_version < "0.16-0":
        logger.info(f"Migrating securevu config from {previous_version} to 0.16-0...")
    ...
    if previous_version < "0.17-0":
        logger.info(f"Migrating securevu config from {previous_version} to 0.17-0...")
    ...
    if previous_version < "0.18-0":
        logger.info(f"Migrating securevu config from {previous_version} to 0.18-0...")
        new_config = migrate_018_0(config)
        with open(config_file, "w") as f:
            yaml.dump(new_config, f)
        previous_version = "0.18-0"

    logger.info("Finished securevu config migration...")


def migrate_014(config: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """Handle migrating securevu config to 0.14"""
...
def migrate_015_0(config: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """Handle migrating securevu config to 0.15-0"""
    new_config = config.copy()
...
def migrate_015_1(config: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """Handle migrating securevu config to 0.15-1"""
    new_config = config.copy()
...
def migrate_016_0(config: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """Handle migrating securevu config to 0.16-0"""
    new_config = config.copy()
...
def migrate_017_0(config: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """Handle migrating securevu config to 0.17-0"""
    new_config = config.copy()
...
def migrate_018_0(config: dict[str, dict[str, Any]]) -> dict[str, dict[str, Any]]:
    """Handle migrating securevu config to 0.18-0"""
    new_config = config.copy()
...
def apply_section_update(camera_config, section: str, update: dict) -> str | None:
    ...
    are recomputed.  This mirrors the logic in SecureVuConfig.post_validation.
    ...
    """
    from securevu.config.config import RuntimeFilterConfig, RuntimeMotionConfig
