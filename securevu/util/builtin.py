...
from securevu.const import REGEX_HTTP_CAMERA_USER_PASS, REGEX_RTSP_CAMERA_USER_PASS
...
def update_yaml_file_bulk(file_path: str, updates: dict[str, Any]):
    ...
    try:
        ...
        logger.error(
            f"Unable to read from SecureVu config file {file_path}. Make sure it exists and is readable."
        )
        return
    ...
    try:
        ...
        logger.error(f"Unable to write to SecureVu config file {file_path}: {e}")
...
def clear_and_unlink(file: Path, missing_ok: bool = True) -> None:
    ...
    # empty contents of file before unlinking https://github.com/SecureCorp-Mexico/SecureVu2/issues/4769
    with open(file, "w"):
        pass
