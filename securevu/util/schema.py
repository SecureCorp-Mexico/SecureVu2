"""JSON schema utilities for SecureVu."""
...
def get_config_schema(config_class: type[BaseModel]) -> dict[str, Any]:
    """
    Returns the JSON schema for SecureVuConfig with polymorphic detectors.

    This utility patches the SecureVuConfig schema to include the full polymorphic
    ...
    from securevu.detectors import DetectorConfig

    # Get the base schema for SecureVuConfig
    ...
    # Ensure $defs exists in SecureVuConfig schema
    if "$defs" not in schema:
        schema["$defs"] = {}

    # Merge $defs from DetectorConfig into SecureVuConfig schema
