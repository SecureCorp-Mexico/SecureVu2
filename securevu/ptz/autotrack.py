...
from securevu.config import CameraConfig, SecureVuConfig, ZoomingModeEnum
...
from securevu.util.builtin import update_yaml_file_bulk
from securevu.util.config import find_config_file
from securevu.util.image import SharedMemoryFrameManager, intersection_over_union
    ...
    def _calculate_move_coefficients(self, camera, calibration=False):
        ...
        if calibration or (
            ...
            if not coefficients_valid:
                logger.warning(
                    f"{camera}: Autotracking calibration failed. See the SecureVu documentation."
                )
                return False
