...
from securevu.config import CameraConfig, DetectConfig, LoggerConfig, ModelConfig
...
from securevu.log import LogPipe
from securevu.motion import MotionDetector
from securevu.motion.improved_motion import ImprovedMotionDetector
from securevu.object_detection.base import RemoteObjectDetector
from securevu.ptz.autotrack import ptz_moving_at_frame_time
from securevu.track import ObjectTracker
from securevu.track.norfair_tracker import NorfairTracker
from securevu.track.tracked_object import TrackedObjectAttribute
from securevu.util.builtin import EventsPerSecond
from securevu.util.image import (
    FrameManager,
    SharedMemoryFrameManager,
    draw_box_with_label,
)
from securevu.util.object import (
    create_tensor_input,
...
from securevu.util.time import get_tomorrow_at_time
...
class CameraCapture(SecureVuProcess):
    def __init__(
        self,
        config: CameraConfig,
        shm_frame_count: int,
        ...
        super().__init__(
            stop_event,
            PROCESS_PRIORITY_HIGH,
            name=f"securevu.capture:{config.name}",
            daemon=True,
        )
        self.config = config
        self.shm_frame_count = shm_frame_count
...
class CameraTracker(SecureVuProcess):
    def __init__(
        self,
        config: CameraConfig,
        ...
        super().__init__(
            stop_event,
            PROCESS_PRIORITY_HIGH,
            name=f"securevu.process:{config.name}",
            daemon=True,
        )
        self.config = config
