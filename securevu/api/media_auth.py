...
from securevu.const import EXPORT_DIR
from securevu.models import Export, User
...
def resolve_media_uri(
    uri: str, securevu_config: SecureVuConfig | None = None
) -> tuple[MediaAuthResolution, str | None]:
    """Classify a URI and return the owning camera if applicable.

    `securevu_config` is used to disambiguate clip/review filenames whose
    ...
    if root == "clips":
        return _resolve_clip(parts, securevu_config)
    if root == "exports":
...
def _resolve_clip(
    parts: list[str], securevu_config: SecureVuConfig | None
) -> tuple[MediaAuthResolution, str | None]:
    ...
    if second == "review":
        if len(parts) == 2:
            return MediaAuthResolution.LISTING_MULTI_CAMERA, None
        camera = _camera_from_thumb_filename(parts[2], securevu_config)
        if camera:
    ...
    if len(parts) == 2:
        camera = _camera_from_clip_filename(second, securevu_config)
        if camera:
            return MediaAuthResolution.CAMERA, camera
        return MediaAuthResolution.UNRESOLVED_MEDIA, None
...
def _longest_prefix_camera(
    stem: str, securevu_config: SecureVuConfig | None
) -> str | None:
    if securevu_config is None:
        return None
    for cam in sorted(securevu_config.cameras.keys(), key=len, reverse=True):
...
def _camera_from_clip_filename(
    filename: str, securevu_config: SecureVuConfig | None
) -> str | None:
    ...
    return _longest_prefix_camera(stem, securevu_config)
...
def check_camera_access(
    ...
    roles_dict = securevu_config.auth.roles
...
def is_role_restricted(role: str, securevu_config: SecureVuConfig) -> bool:
    """True if `role` has a non-empty allow-list (i.e. not full-access)."""
    if role == "admin":
        return False
    return bool(securevu_config.auth.roles.get(role))


def deny_response_for_media_uri(
    original_url: str | None, role: str | None, securevu_config: SecureVuConfig
) -> int | None:
    ...
    resolution, camera = resolve_media_uri(path, securevu_config)
    ...
    if not is_role_restricted(role, securevu_config):
        return None
    ...
    if resolution == MediaAuthResolution.CAMERA:
        if camera and check_camera_access(role, camera, securevu_config):
            return None
        return 403
