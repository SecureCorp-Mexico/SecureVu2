...
class DebugReplayStatusResponse(BaseModel):
    ...
    debug_replay job (see securevu.jobs.debug_replay); the
...
async def start_debug_replay(request: Request, body: DebugReplayStartBody):
    ...
    internal_port = request.app.securevu_config.networking.listen.internal
...
async def start_debug_replay_from_export(
    ...
    properties = await get_video_properties(
        request.app.securevu_config.ffmpeg, export.video_path, get_duration=True
    )
...
def get_debug_replay_status(request: Request):
    ...
    if replay_manager.active and replay_camera:
        ...
        if frame is not None:
            ...
            camera_config = request.app.securevu_config.cameras.get(replay_camera)
            retry_interval = 10.0
