...
from securevu.api.defs.query.media_query_parameters import (
    Extension,
...
from securevu.camera.state import CameraState
from securevu.config import SecureVuConfig
from securevu.config.camera.snapshots import SnapshotsConfig
...
from securevu.models import Event, Previews, Recordings, Regions, ReviewSegment
from securevu.output.preview import get_most_recent_preview_frame
from securevu.track.object_processing import TrackedObjectProcessor
from securevu.util.file import (
    get_event_snapshot_bytes,
    get_event_snapshot_path,
    get_event_thumbnail_bytes,
    load_event_snapshot_image,
)
from securevu.util.image import get_image_from_recording, get_image_quality_params
from securevu.util.media import get_keyframe_before
from securevu.util.object import create_empty_regions_grid
...
async def mjpeg_feed(
    ...
    if camera_name in request.app.securevu_config.cameras:
...
async def latest_frame(
    ...
    camera_config = request.app.securevu_config.cameras.get(camera_name)
    if camera_config is not None:
            ...
            if frame is None or not is_offline:
                ...
                if request.app.camera_error_image is None:
                    error_image = glob.glob(
                        os.path.join(INSTALL_DIR, "securevu/images/camera-error.jpg")
                    )
        ...
        if is_offline:
            headers["X-SecureVu-Offline"] = "true"
        ...
        and request.app.securevu_config.birdseye.enabled
        and request.app.securevu_config.birdseye.restream
    ):
        frame = cv2.cvtColor(
...
async def get_snapshot_from_recording(
    ...
    if camera_name not in request.app.securevu_config.cameras:
        return JSONResponse(
            content={"success": False, "message": "Camera not found"},
            status_code=404,
        )
...
async def submit_recording_snapshot_to_plus(
    ...
    try:
        ...
        await asyncio.to_thread(
            request.app.securevu_config.plus_api.upload_image, nd, camera_name
        )
...
async def event_snapshot(
    ...
    try:
        ...
        snapshot_settings = _resolve_snapshot_settings(
            request.app.securevu_config.cameras[event.camera].snapshots, params
        )
        jpg_bytes, frame_time = get_event_snapshot_bytes(
            ...
            timestamp_style=request.app.securevu_config.cameras[
                event.camera
            ].timestamp_style,
            colormap=request.app.securevu_config.model.colormap,
        )
    except DoesNotExist:
...
def grid_snapshot(
    ...
    if camera_name in request.app.securevu_config.cameras:
        detect = request.app.securevu_config.cameras[camera_name].detect
        frame_processor: TrackedObjectProcessor = request.app.detected_frames_processor
        frame = frame_processor.get_current_frame(camera_name, {})
        retry_interval = float(
            request.app.securevu_config.cameras.get(camera_name).ffmpeg.retry_interval
            or 10
        )
...
async def event_snapshot_clean(request: Request, event_id: str, download: bool = False):
    ...
    try:
        ...
        await require_camera_access(event.camera, request=request)
        snapshot_config = request.app.securevu_config.cameras[event.camera].snapshots
