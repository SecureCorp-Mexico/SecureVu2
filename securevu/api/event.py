...
from securevu.api.defs.query.events_query_parameters import (
    DEFAULT_TIME_RANGE,
    EventsQueryParams,
    EventsSearchQueryParams,
    EventsSummaryQueryParams,
)
from securevu.api.defs.query.regenerate_query_parameters import (
    RegenerateQueryParameters,
)
from securevu.api.defs.request.events_body import (
...
from securevu.api.defs.response.event_response import (
    EventCreateResponse,
    EventMultiDeleteResponse,
    EventResponse,
    EventUploadPlusResponse,
)
from securevu.api.defs.response.generic_response import GenericResponse
from securevu.api.defs.tags import Tags
from securevu.comms.event_metadata_updater import EventMetadataTypeEnum
...
from securevu.models import Event, ReviewSegment, Timeline, Trigger
from securevu.track.object_processing import TrackedObject
from securevu.util.file import get_event_thumbnail_bytes, load_event_snapshot_image
from securevu.util.path import get_trigger_thumbnail_path, safe_join
from securevu.util.time import get_dst_transitions, get_tz_modifiers
...
def events_search(
    ...
    if not request.app.securevu_config.semantic_search.enabled:
        return JSONResponse(
            content=(
                {
                    "success": False,
...
@router.post(
    ...
    dependencies=[Depends(require_role(["admin"]))],
    summary="Send event to SecureVu+",
...
async def send_to_plus(request: Request, event_id: str, body: SubmitPlusBody = None):
    ...
    try:
        plus_id = await asyncio.to_thread(
            request.app.securevu_config.plus_api.upload_image, image, event.camera
        )
    ...
    if include_annotation is not None:
        ...
        try:
            await asyncio.to_thread(
                request.app.securevu_config.plus_api.add_annotation,
                event.plus_id,
                box,
                event.label,
            )
...
@router.put(
    ...
    response_model=EventUploadPlusResponse,
    dependencies=[Depends(require_role(["admin"]))],
    summary="Submit false positive to SecureVu+",
...
async def false_positive(request: Request, event_id: str):
    ...
    if event.false_positive:
        message = "False positive already submitted to SecureVu+"
        logger.error(message)
        return JSONResponse(
            content=({"success": False, "message": message}), status_code=400
        )
    ...
    try:
        await asyncio.to_thread(
            request.app.securevu_config.plus_api.add_false_positive,
            event.plus_id,
            region,
            box,
            score,
...
async def set_attributes(
    ...
    for (
        model_key,
        model_config,
    ) in request.app.securevu_config.classification.custom.items():
...
async def set_description(
    ...
    if context is not None:
        if len(new_description) > 0:
            ...
            if request.app.securevu_config.semantic_search.enabled:
...
async def regenerate_description(
    ...
    camera_config = request.app.securevu_config.cameras[event.camera]
...
def create_event(
    ...
    if not camera_name or not request.app.securevu_config.cameras.get(camera_name):
        return JSONResponse(
            content=(
...
def create_trigger_embedding(
    ...
    try:
        ...
        Trigger.create(
            ...
            model=request.app.securevu_config.semantic_search.model,
