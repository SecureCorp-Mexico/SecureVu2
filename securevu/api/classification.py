...
from securevu.api.auth import require_full_camera_access, require_role
from securevu.api.defs.request.classification_body import (
...
from securevu.api.defs.response.classification_response import (
...
from securevu.util.file import get_event_snapshot
from securevu.util.path import safe_join, sanitize_path_component
...
def reclassify_face(request: Request, body: dict = None):
    if not request.app.securevu_config.face_recognition.enabled:
        return JSONResponse(
            status_code=400,
...
def train_face(request: Request, name: str, body: dict = None):
    ...
    if training_file_name:
        ...
        detect_config: DetectConfig = request.app.securevu_config.cameras[
            event.camera
        ].detect
...
def register_face(request: Request, name: str, file: UploadFile):
    ...
    if not isinstance(result, dict):
        return JSONResponse(
            ...
            content={
                "success": False,
                "message": "Could not process request. Try restarting SecureVu.",
            },
        )
...
def rename_face(request: Request, old_name: str, body: RenameFaceBody):
    ...
    try:
        ...
        return JSONResponse(
            status_code=400,
            content={
                "message": "Error renaming face. Check SecureVu logs.",
                "success": False,
            },
        )
...
def reprocess_license_plate(request: Request, event_id: str):
    if not request.app.securevu_config.lpr.enabled:
