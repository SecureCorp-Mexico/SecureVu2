...
from securevu.api.config_util import swap_runtime_config
from securevu.api.defs.request.app_body import CameraSetBody
...
from securevu.config.env import substitute_securevu_vars
from securevu.models import User
from securevu.util.builtin import clean_camera_user_pass, get_record_segment_time
from securevu.util.camera_cleanup import cleanup_camera_db, cleanup_camera_files
from securevu.util.config import find_config_file
from securevu.util.image import run_ffmpeg_snapshot
...
def go2rtc_camera_stream(request: Request, stream_name: str):
    ...
    if not r.ok:
        camera_config = request.app.securevu_config.cameras.get(stream_name)
...
def go2rtc_add_stream(request: Request, stream_name: str, src: str = ""):
    ...
    try:
        ...
        if src:
            try:
                resolved_src = substitute_securevu_vars(src)
            except KeyError:
                resolved_src = src
...
def ffprobe(request: Request, paths: str = "", detailed: bool = False):
    ...
    if path_param.startswith("camera"):
        ...
        if camera not in request.app.securevu_config.cameras.keys():
            return JSONResponse(
                content=(
        ...
        if not request.app.securevu_config.cameras[camera].enabled:
            return JSONResponse(
        ...
        paths = map(
            ...
            request.app.securevu_config.cameras[camera].ffmpeg.inputs,
        )
    ...
    for path in paths:
        ffprobe = ffprobe_stream(
            request.app.securevu_config.ffmpeg, path.strip(), detailed=detailed
        )
                ...
                for test_uri in to_test:
                    try:
                        probe = ffprobe_stream(
                            request.app.securevu_config.ffmpeg, test_uri, detailed=False
                        )
...
async def delete_camera(
    ...
    """
    securevu_config: SecureVuConfig = request.app.securevu_config

    if camera_name not in securevu_config.cameras:
        return JSONResponse(
            content={
                "success": False,
    ...
    old_camera_config = securevu_config.cameras[camera_name]
...
def camera_set(
    ...
    elif camera_name not in securevu_config.cameras:
        return JSONResponse(
            content={
                "success": False,
                "message": f"Camera '{camera_name}' not found",
