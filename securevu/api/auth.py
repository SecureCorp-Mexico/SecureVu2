...
from securevu.api.media_auth import (
    check_camera_access,
    deny_response_for_media_uri,
    is_role_restricted,
)
from securevu.config import AuthConfig, ProxyConfig
from securevu.const import CONFIG_DIR, JWT_SECRET_ENV_VAR, PASSWORD_HASH_ALGORITHM
from securevu.models import User
    ...
    async def admin_checker(request: Request):
        ...
        try:
            if path.startswith("/"):
                ...
                if (
                    first_segment
                    and first_segment in request.app.securevu_config.cameras
                ):
                    return
        except Exception:
            pass
...
def first_time_login(request: Request):
    ...
    """
    auth_config = request.app.securevu_config.auth
...
def get_remote_addr(request: Request):
    ...
    trusted_proxies = []
    for proxy in request.app.securevu_config.auth.trusted_proxies:
        try:
...
def set_jwt_cookie(response: Response, cookie_name, encoded_jwt, max_age, secure):
    ...
    # iframes, breaking embedded views such as the Home Assistant SecureVu card.
    # CSRF is instead mitigated by requiring a custom X-CSRF-TOKEN header, which
    # cross-origin pages cannot set without a CORS preflight that SecureVu never
...
def require_role(required_roles: list[str]):
    async def role_checker(request: Request):
        proxy_config: ProxyConfig = request.app.securevu_config.proxy
        config_roles = list(request.app.securevu_config.auth.roles.keys())
...
def auth(request: Request):
    auth_config: AuthConfig = request.app.securevu_config.auth
    proxy_config: ProxyConfig = request.app.securevu_config.proxy

    success_response = Response("", status_code=202)

    # dont require auth if the request is on the internal port
    # this header is set by SecureVu's nginx proxy, so it cant be spoofed.
    ...
    securevu_config = request.app.securevu_config
    ...
    if not auth_config.enabled:
        ...
        deny_status = deny_response_for_media_uri(original_url, role, securevu_config)
    ...
    JWT_COOKIE_NAME = request.app.securevu_config.auth.cookie_name
    JWT_COOKIE_SECURE = request.app.securevu_config.auth.cookie_secure
    JWT_REFRESH = request.app.securevu_config.auth.refresh_time
    JWT_SESSION_LENGTH = request.app.securevu_config.auth.session_length
...
def profile(request: Request):
    ...
    all_camera_names = set(request.app.securevu_config.cameras.keys())
    roles_dict = request.app.securevu_config.auth.roles
...
def login(request: Request, body: AppPostLoginBody):
    if not request.app.securevu_config.auth.enabled:
        return JSONResponse(
    ...
    if verify_password(password, password_hash):
        ...
        config_roles_set = set(request.app.securevu_config.auth.roles.keys())
        ...
        if role == "admin":
            request.app.securevu_config.auth.admin_first_time_login = False
...
def create_user(
    ...
    HASH_ITERATIONS = request.app.securevu_config.auth.hash_iterations
    config_roles = list(request.app.securevu_config.auth.roles.keys())
...
async def require_camera_access(
    ...
    if check_camera_access(role, camera_name, securevu_config):
        return

    all_camera_names = set(securevu_config.cameras.keys())
    allowed_cameras = User.get_allowed_cameras(
        role, securevu_config.auth.roles, all_camera_names
    )
    raise HTTPException(
        status_code=403,
...
def deny_response_for_go2rtc_stream(
    ...
    if not role or not is_role_restricted(role, securevu_config):
        return None
    ...
    allowed_cameras = set(
        User.get_allowed_cameras(
            role,
            securevu_config.auth.roles,
            set(securevu_config.cameras.keys()),
        )
    )
