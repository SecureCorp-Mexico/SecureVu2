...
from securevu.api.auth import (
    allow_any_authenticated,
    allow_public,
    get_allowed_cameras_for_filter,
    require_role,
)
from securevu.api.config_util import (
...
from securevu.const import REDACTED_CREDENTIAL_SENTINEL
from securevu.ffmpeg_presets import FFMPEG_HWACCEL_VAAPI, _gpu_selector
from securevu.genai import PROVIDERS, load_providers
from securevu.jobs.media_sync import (
    get_current_media_sync_job,
    get_media_sync_job_by_id,
    start_media_sync_job,
)
from securevu.models import Event, Timeline
from securevu.stats.prometheus import get_metrics, update_metrics
from securevu.types import JobStatusTypesEnum
from securevu.util.builtin import (
...
from securevu.util.config import (
    apply_section_update,
    find_config_file,
    redact_credential,
)
from securevu.util.schema import get_config_schema
from securevu.util.services import (
    get_nvidia_driver_info,
    process_logs,
    restart_securevu,
    vainfo_hwaccel,
)
from securevu.util.time import get_tz_modifiers
...
def is_healthy():
    return "SecureVu is running. Alive and healthy!"


@router.get("/config/schema.json", dependencies=[Depends(allow_public())])
def config_schema(request: Request):
    return JSONResponse(content=get_config_schema(SecureVuConfig))
...
async def genai_probe(request: Request, body: GenAIProbeBody):
    ...
    if api_key == REDACTED_CREDENTIAL_SENTINEL:
        saved_cfg = (
            request.app.securevu_config.genai.get(body.name) if body.name else None
        )
...
def config(request: Request):
    config_obj: SecureVuConfig = request.app.securevu_config
    ...
    for camera_name, camera in request.app.securevu_config.cameras.items():
    ...
    config["plus"] = {"enabled": request.app.securevu_config.plus_api.is_active()}
    ...
    for detector_config in config["detectors"].values():
        detector_config["model"]["labelmap"] = (
            request.app.securevu_config.model.merged_labelmap
        )
...
def config_save(save_option: str, body: Any = Body(media_type="text/plain")):
    ...
    try:
        ...
        SecureVuConfig.parse_yaml(new_config)
        ...
        return JSONResponse(
            content=(
                {
                    "success": False,
                    "message": "Your configuration is invalid.\nSee the official documentation at docs.secure.vu.\n\n"
                    + "\n".join(error_message),
                }
            ),
            status_code=400,
    ...
    try:
        ...
        return JSONResponse(
            content=(
                {
                    "success": False,
                    "message": "Could not write config file, be sure that SecureVu has write permission on the config file.",
                }
            ),
            status_code=400,
        )
    ...
    if save_option == "restart":
        try:
            ...
            logging.error(f"Error restarting SecureVu: {e}")
            return JSONResponse(
                content=(
                    {
                        "success": True,
                        "message": "Config successfully saved, unable to restart SecureVu",
                    }
                ),
                status_code=200,
            )
...
def _restore_masked_camera_paths(config_data: dict, config: SecureVuConfig) -> None:
...
def _config_set_in_memory(request: Request, body: AppConfigSetBody) -> JSONResponse:
    ...
    try:
        updates = {}
        if body.config_data:
            _restore_masked_camera_paths(body.config_data, request.app.securevu_config)
    ...
    try:
        with lock:
            ...
            try:
                ...
                if query_string:
                    ...
                    _restore_masked_camera_paths(
                        body.config_data, request.app.securevu_config
                    )
                    updates = flatten_config_data(body.config_data)
                ...
                try:
                    config = SecureVuConfig.parse(new_raw_config)
                except ValidationError as e:
                    with open(config_file, "w") as f:
            ...
            if body.requires_restart == 0 or body.update_topic:
                old_config: SecureVuConfig = request.app.securevu_config
                ...
                if body.update_topic:
                    if body.update_topic.startswith("config/cameras/"):
                        ...
                        # global topic the global object. SecureVuConfig.parse
...
async def logs(
    service: str = Path(enum=["securevu", "nginx", "go2rtc"]),
    ...
    log_locations = {
        "securevu": "/dev/shm/logs/securevu/current",
    ...
    try:
        ...
        return JSONResponse(
            content=(
                {
                    "success": False,
                    "message": "Unable to restart SecureVu.",
                }
            ),
            status_code=500,
        )
...
def plusModels(request: Request, filterByCurrentModelDetector: bool = False):
    if not request.app.securevu_config.plus_api.is_active():
        return JSONResponse(
            content=({"success": False, "message": "SecureVu+ is not enabled"}),
            status_code=400,
        )

    models: dict[Any, Any] = request.app.securevu_config.plus_api.get_models()
    ...
    # current model type
    modelType = request.app.securevu_config.model.model_type

    # current detectorType for comparing to supportedDetectors
    detectorType = list(request.app.securevu_config.detectors.values())[0].type
