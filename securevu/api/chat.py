...
from securevu.api.chat_util import (
    chunk_content,
    distance_to_score,
...
from securevu.api.defs.query.events_query_parameters import EventsQueryParams
from securevu.api.defs.request.chat_body import ChatCompletionRequest
from securevu.api.defs.response.chat_response import (
    ChatCompletionResponse,
    ChatMessageResponse,
    ToolCall,
)
from securevu.api.defs.tags import Tags
from securevu.api.event import _build_attribute_filter_clause, events
from securevu.config import SecureVuConfig
from securevu.config.classification import SemanticSearchModelEnum
from securevu.genai.prompts import (
    build_chat_system_prompt,
    get_attribute_classifications,
    get_tool_definitions,
)
from securevu.genai.utils import build_assistant_message_for_conversation
from securevu.jobs.vlm_watch import (
    get_vlm_watch_job,
    start_vlm_watch_job,
    stop_vlm_watch_job,
)
...
def get_tools(request: Request) -> JSONResponse:
    ...
    config = request.app.securevu_config
...
def _embeddings_language(config: SecureVuConfig) -> Literal["english", "multi"]:
...
async def _execute_find_similar_objects(
    ...
    if not getattr(config.semantic_search, "enabled", False):
        return {
            ...
            "message": (
                ...
                "Enable it in the SecureVu config under semantic_search."
            ),
        }
    ...
    if zones:
        zones = _resolve_zones(
            zones, request.app.securevu_config, cameras or list(allowed_cameras)
        )
    ...
    # version (see securevu/embeddings/__init__.py). Mirror the pattern used by
    # securevu/api/event.py events_search: fetch top-k globally, then intersect
    ...
    if zones:
        # Mirror the pattern used by securevu/api/event.py for JSON-array zone match.
        zone_clauses = [Event.zones.cast("text") % f'*"{zone}"*' for zone in zones]
        clauses.append(reduce(operator.or_, zone_clauses))
...
async def execute_tool(
    ...
    SecureVu operations, returning results in a format the LLM can understand.
    """
...
async def _execute_get_live_context(
    ...
    if camera not in request.app.securevu_config.cameras:
        return {
...
async def _get_live_frame_image_url(
    ...
    if (
        ...
        or camera not in request.app.securevu_config.cameras
    ):
        return None
    try:
        frame_processor = request.app.detected_frames_processor
...
async def _execute_set_camera_state(
    ...
    if camera == "*":
        cameras = list(securevu_config.cameras.keys())
    elif camera not in securevu_config.cameras:
...
def _execute_get_recap(
    ...
    from securevu.models import ReviewSegment
...
@router.post(
    ...
    description=(
        ...
        "The LLM can call SecureVu tools to answer questions about your cameras and events."
    ),
)
async def chat_completion(
    ...
    if not genai_client:
        return JSONResponse(
            content={
                "error": "GenAI is not configured. Please configure a GenAI provider in your SecureVu config.",
            },
            status_code=400,
        )
