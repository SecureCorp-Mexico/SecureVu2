...
shaping so provider clients in :mod:`securevu.genai.plugins` only handle
transport.
"""
...
from securevu.config import CameraConfig, SecureVuConfig
from securevu.config.classification import ObjectClassificationType
from securevu.config.ui import UnitSystemEnum
...
def get_attribute_classifications(config: SecureVuConfig) -> list[dict[str, Any]]:
...
def get_tool_definitions(
    ...
    """
    Get OpenAI-compatible tool definitions for SecureVu.
    ...
    search_objects_description = (
        "Search the historical record of detected objects in SecureVu. "
        ...
        "An 'object' in SecureVu represents a tracked detection (e.g., a person, package, car).\n\n"
...
def build_chat_system_prompt(
    ...
    return f"""You are a helpful assistant for SecureVu, a security camera NVR system. You help users answer questions about their cameras, detected objects, and events.
