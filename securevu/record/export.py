...
from securevu.config import FfmpegConfig, SecureVuConfig
...
from securevu.ffmpeg_presets import (
    EncodeTypeEnum,
    parse_preset_hardware_acceleration_encode,
)
from securevu.models import Export, Previews, Recordings, ReviewSegment
...
class RecordingExporter(threading.Thread):
    ...
    def get_record_export_command(
        ...
        # add metadata
        title = f"SecureVu Recording for {self.camera}, {self.get_datetime_from_timestamp(self.start_time)} - {self.get_datetime_from_timestamp(self.end_time)}"
    ...
    def get_preview_export_command(
        ...
        # add metadata
        title = f"SecureVu Preview for {self.camera}, {self.get_datetime_from_timestamp(self.start_time)} - {self.get_datetime_from_timestamp(self.end_time)}"
        creation_time = datetime.datetime.fromtimestamp(
            self.start_time, tz=datetime.UTC
        ).strftime("%Y-%m-%dT%H:%M:%S.%fZ")
        ffmpeg_cmd.extend(
