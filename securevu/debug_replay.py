...
securevu.jobs.debug_replay. This module owns only session presence
...
from securevu.jobs.debug_replay import (
...
class DebugReplayManager:
    ...
    def publish_camera(
        ...
        """
        source_config = securevu_config.cameras[source_camera]
        ...
        try:
            new_config = SecureVuConfig.parse_object(config_data)
        except Exception as e:
            raise RuntimeError(f"Failed to validate replay camera config: {e}") from e
        securevu_config.cameras[replay_name] = new_config.cameras[replay_name]
    ...
    def stop(
        ...
        with self._lock:
            ...
            if replay_name is not None and replay_name in securevu_config.cameras:
                config_publisher.publish_update(
                    CameraConfigUpdateTopic(CameraConfigUpdateEnum.remove, replay_name),
                    securevu_config.cameras[replay_name],
                )
                securevu_config.cameras.pop(replay_name, None)
