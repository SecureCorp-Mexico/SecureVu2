...
import type { CameraConfig, SecureVuConfig } from "@/types/securevuConfig";
import type {
  ConfigSectionData,
  JsonObject,
  JsonValue,
 ...
 * the backend applies to attribute labels (face, license_plate, SecureVu+
 * couriers) — see `securevu/config/config.py`.
 */
...
export function buildClonedCameraPayloads({
  ...
  const syntheticTargetCamera = targetIsNew
    ? ({
      ...
      } as unknown as SecureVuConfig["cameras"][string])
    : ((fullConfig.cameras?.[target]
        ? cloneDeep(fullConfig.cameras[target])
        : { enabled: true }) as unknown as SecureVuConfig["cameras"][string]);
  ...
  const syntheticConfig: SecureVuConfig = {
    ...fullConfig,
    cameras: {
      ...fullConfig.cameras,
