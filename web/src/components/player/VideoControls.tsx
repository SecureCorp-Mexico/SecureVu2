...
import SecureVuPlusIcon from "../icons/SecureVuPlusIcon";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
type SecureVuPlusUploadButtonProps = {
  video?: HTMLVideoElement | null;
  onOpen: () => void;
  onClose: () => void;
  onUploadFrame: () => void;
...
function SecureVuPlusUploadButton({
  video,
  onOpen,
  onClose,
  onUploadFrame,
  ...
  return (
    ...
    >
      ...
      >
        <AlertDialogHeader>
          <AlertDialogTitle>{t("submitSecureVuPlus.title")}</AlertDialogTitle>
        </AlertDialogHeader>
        {previewError ? (
          <div className="flex aspect-video w-full flex-col items-center justify-center gap-2 text-center text-muted-foreground">
            ...
            <span>{t("submitSecureVuPlus.previewError")}</span>
          </div>
        ) : (
          <img
