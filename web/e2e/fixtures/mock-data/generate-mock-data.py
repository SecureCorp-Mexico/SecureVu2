...
Run from the repo root:
    PYTHONPATH=/workspace/securevu python3 web/e2e/fixtures/mock-data/generate-mock-data.py

Strategy:
  - SecureVuConfig: instantiate the Pydantic config model, then model_dump()
...
def generate_config():
    """Generate SecureVuConfig from the Python backend model."""
    from securevu.config import SecureVuConfig

    config = SecureVuConfig.model_validate_json(
        json.dumps(
            {
                "mqtt": {"host": "mqtt"},
                "cameras": {
...
def generate_config_schema():
    """Generate the JSON Schema for SecureVuConfig from the backend model.
    ...
    return SecureVuConfig.model_json_schema()


def generate_reviews():
    """Generate ReviewSegmentResponse[] validated against Pydantic + Peewee."""
    from securevu.api.defs.response.review_response import ReviewSegmentResponse
...
def generate_events():
    ...
    from securevu.api.defs.response.event_response import EventResponse
...
def generate_exports():
    ...
    from securevu.api.defs.response.export_response import ExportModel
...
def generate_cases():
    ...
    from securevu.api.defs.response.export_case_response import ExportCaseModel
