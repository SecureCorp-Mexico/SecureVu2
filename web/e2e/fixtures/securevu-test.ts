...
/**
 * Extended Playwright test fixture with SecureVuApp.
 *
 * Every test imports `test` and `expect` from this file instead of
 * @playwright/test directly. The `securevuApp` fixture provides a
 * fully mocked SecureVu frontend ready for interaction.
 *
...
type SecureVuFixtures = {
  securevuApp: SecureVuApp;
  /**
...
export const test = base.extend<SecureVuFixtures>({
  ...
  securevuApp: async ({ page, errorCollector }, use, testInfo) => {
    // Reference the collector so its `use()` runs and teardown fires
    void errorCollector;
    const app = new SecureVuApp(page, testInfo.project.name);
