...
test.describe("Explore — search @high", () => {
  test("search input accepts text and clears", async ({ securevuApp }) => {
    // Enable semantic search so InputWithTags renders.
    await securevuApp.installDefaults({ config: SEMANTIC_SEARCH_CONFIG });
    await securevuApp.goto("/explore");
    const searchInput = securevuApp.page.locator("input").first();
  ...
  test("Enter submission does not crash the page", async ({ securevuApp }) => {
    await securevuApp.installDefaults({ config: SEMANTIC_SEARCH_CONFIG });
    await securevuApp.goto("/explore");
    const searchInput = securevuApp.page.locator("input").first();
    await expect(searchInput).toBeVisible({ timeout: 10_000 });
...
test.describe("Explore — filters (desktop) @high", () => {
  test.skip(({ securevuApp }) => securevuApp.isMobile, "Desktop popovers");

  test("Cameras popover lists configured cameras", async ({ securevuApp }) => {
    // Navigate with a labels filter param so the filter bar renders.
    await securevuApp.goto("/explore?labels=person");
    ...
    const camerasBtn = securevuApp.page.getByLabel("Cameras Filter").first();
    ...
    await expect(securevuApp.page.getByText("Front Door")).toBeVisible();
  });

  test("Labels filter lists labels from config", async ({ securevuApp }) => {
    ...
    // remains "Labels".
    const labelsBtn = securevuApp.page.getByLabel("Labels").first();
    ...
    const overlay = securevuApp.page.locator(
  ...
  test("Sub-labels filter renders inside More Filters dialog", async ({
    ...
    await securevuApp.page.route("**/api/sub_labels**", (route) =>
      route.fulfill({ json: [] }),
    );
    await securevuApp.goto("/explore?labels=person");
    const moreBtn = securevuApp.page.getByLabel("More Filters").first();
    ...
    await expect(
      securevuApp.page.getByText(/sub.?label/i).first(),
    ).toBeVisible();
    await securevuApp.page.keyboard.press("Escape");
  });
  ...
  test("Zones filter lists configured zones inside More Filters dialog", async ({
    ...
    await expect(overlay.first()).toBeVisible({ timeout: 3_000 });
    await expect(securevuApp.page.getByText(/front.?yard/i)).toBeVisible();
    await securevuApp.page.keyboard.press("Escape");
  });
});
...
test.describe("Explore — content @high", () => {
  test("page renders with mock events", async ({ securevuApp }) => {
    await securevuApp.goto("/explore");
    await expect(securevuApp.page.locator("#pageRoot")).toBeVisible({
      timeout: 10_000,
    });
    await expect(
      securevuApp.page.locator("#pageRoot button").first(),
    ).toBeVisible({ timeout: 10_000 });
  });

  test("empty events renders without crash", async ({ securevuApp }) => {
    await securevuApp.installDefaults({ events: [] });
    await securevuApp.goto("/explore");
    await expect(securevuApp.page.locator("#pageRoot")).toBeVisible({
      timeout: 10_000,
    });
  ...
  test("search fires a /api/events request with the query", async ({
    ...
    securevuApp.page.on("request", (req) => {
      const url = req.url();
...
test.describe("Explore — similarity search (desktop) @high", () => {
  ...
  test("URL similarity search params fetch events", async ({ securevuApp }) => {
    const eventsRequests: string[] = [];
    securevuApp.page.on("request", (req) => {
      const url = req.url();
      if (/\/api\/events/.test(url)) eventsRequests.push(url);
...
// ---------------------------------------------------------------------------
// SecureVu+ submission — desktop only
// The detail dialog's previous/next arrows only render on desktop.
// ---------------------------------------------------------------------------

test.describe("Explore — SecureVu+ submission (desktop) @high", () => {
  test.skip(
    ({ securevuApp }) => securevuApp.isMobile,
  ...
  test("in-flight submission does not mark the next tracked object as submitted", async ({
    ...
    await securevuApp.installDefaults({ config: { plus: { enabled: true } } });
    const page = securevuApp.page;
