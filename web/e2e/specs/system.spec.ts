...
test.describe("System — tabs @medium", () => {
  test("general tab is active by default via #general hash", async ({
    ...
    await expect(securevuApp.page.getByLabel("Select storage")).toBeVisible();
    await expect(securevuApp.page.getByLabel("Select cameras")).toBeVisible();
  });
  ...
  test("Storage tab activates and deactivates General", async ({
    ...
    await securevuApp.page.getByLabel("Select storage").click();
    await expect(securevuApp.page.getByLabel("Select storage")).toHaveAttribute(
      "data-state",
      "on",
      { timeout: 5_000 },
    );
  ...
  test("Cameras tab activates", async ({ securevuApp }) => {
    await securevuApp.goto("/system#general");
    await expect(securevuApp.page.getByLabel("Select general")).toHaveAttribute(
      "data-state",
      "on",
    ...
    await securevuApp.page.getByLabel("Select cameras").click();
    await expect(securevuApp.page.getByLabel("Select cameras")).toHaveAttribute(
      "data-state",
      "on",
      { timeout: 5_000 },
    );
  ...
  test("general tab shows version and last-refreshed", async ({
    ...
    await expect(securevuApp.page.getByText("0.15.0-test")).toBeVisible();
    await expect(securevuApp.page.getByText(/Last refreshed/)).toBeVisible();
  });
  ...
  test("storage tab renders content after switching", async ({
    ...
    if (!securevuApp.isMobile) {
      await expect
        .poll(
          async () => (await securevuApp.page.textContent("#pageRoot")) ?? "",
          { timeout: 10_000 },
        )
      ...
      await expect(
        securevuApp.page.getByLabel("Select general"),
...
test.describe("System — RestartDialog @medium", () => {
  ...
  test("cancelling restart leaves body interactive", async ({ securevuApp }) => {
    ...
    const sidebarTriggers = securevuApp.page
      .locator('[role="complementary"] [aria-haspopup="menu"]')
      .or(securevuApp.page.locator('aside [aria-haspopup="menu"]'));
    ...
    for (let i = 0; i < triggerCount; i++) {
      ...
      const restartItem = securevuApp.page
      ...
      await securevuApp.page.keyboard.press("Escape").catch(() => {});
    }
    expect(opened).toBe(true);

    const cancel = securevuApp.page.getByRole("button", { name: /cancel/i });
...
test.describe("System — mobile @medium @mobile", () => {
  ...
  test("tabs render at mobile viewport", async ({ securevuApp }) => {
    await securevuApp.goto("/system#general");
  ...
  test("switching tabs works at mobile viewport", async ({ securevuApp }) => {
    await securevuApp.goto("/system#general");
    await expect(securevuApp.page.getByLabel("Select general")).toHaveAttribute(
      "data-state",
      "on",
