...
async function openRecordingView(securevuApp: SecureVuApp) {
  ...
  await securevuApp.page.route("**/api/*/recordings/coverage**", (route) =>
    route.fulfill({
      json: {
        spans: [
          {
...
async function openDebugReplayForm(securevuApp: SecureVuApp) {
...
async function selectRangeFromTimeline(securevuApp: SecureVuApp) {
  const form = await openDebugReplayForm(securevuApp);
  ...
  await expect(securevuApp.page.locator(".export-start")).toHaveText(
    /\d{1,2}:\d{2}/,
    { timeout: 5_000 },
  );
}
...
async function reselectRange(securevuApp: SecureVuApp) {
  await selectRangeFromTimeline(securevuApp);

  await securevuApp.page
    .getByRole("button", { name: /^cancel$/i })
    .click({ timeout: 5_000 });
  await expect(securevuApp.page.locator(".export-start")).toHaveCount(0);

  const segments = securevuApp.page.locator(".segment[data-segment-id]");
...
async function countHandleLabelChanges(securevuApp: SecureVuApp) {
  return securevuApp.page.evaluate(async () => {
...
test.describe("Debug Replay from History @high", () => {
  test("a reselected range lands once and stays put", async ({
    ...
    securevuApp.page.on("pageerror", (err) => pageErrors.push(err.message));

    await securevuApp.installDefaults();
    await openRecordingView(securevuApp);
    await reselectRange(securevuApp);

    expect(await countHandleLabelChanges(securevuApp)).toBe(0);
    expect(
  ...
  test("dragging a handle after a reselect moves it", async ({
    ...
    const start = securevuApp.page.locator(".export-start");
    ...
    await securevuApp.page.mouse.move(x, y);
    await securevuApp.page.mouse.down();
    await securevuApp.page.mouse.move(x, y - 90, { steps: 12 });
    await securevuApp.page.mouse.up();
