...
test.describe("go2rtc streams settings — ffmpeg compat mode @medium", () => {
  test("preserves unknown fragments like #timeout= in the URL input", async ({
    ...
    await installRawPathsRoute(securevuApp.page, FFMPEG_URL_WITH_TIMEOUT);
    await securevuApp.goto("/settings?page=systemGo2rtcStreams");

    await expect(
      securevuApp.page.getByRole("heading", { name: STREAM_NAME }),
    ).toBeVisible();

    await expandStream(securevuApp.page, STREAM_NAME);

    const urlInput = securevuApp.page.getByPlaceholder(
  ...
  test("preserves repeatable #audio= fallback chain and lets the user add another codec", async ({
    ...
    const audioLabel = securevuApp.page.locator(`label:text-is("Audio")`);
  ...
  test("picking Exclude on the primary row drops the #video= fragment entirely", async ({
    ...
    const videoLabel = securevuApp.page.locator(`label:text-is("Video")`);
    const videoRowsContainer = videoLabel.locator("xpath=../..");
    await videoRowsContainer.getByRole("combobox").first().click();
    await securevuApp.page.getByRole("option", { name: "Exclude" }).click();
