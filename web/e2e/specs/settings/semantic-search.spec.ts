...
test.describe("semantic search model_size @medium", () => {
  test("a provider with a defaulted model_size is not dirty on load", async ({
    ...
    await expect(securevuApp.page.getByText(NOT_APPLICABLE)).toBeVisible();
    ...
    await securevuApp.page.waitForTimeout(1000);
    await expect(securevuApp.page.getByText(UNSAVED)).toBeHidden();
    await expect(
      securevuApp.page.getByRole("button", { name: "Save", exact: true }),
    ).toBeDisabled();
  });
  ...
  test("switching from a configured non-default model_size clears it", async ({
    ...
    await securevuApp.page
      ...
      .click();
    await securevuApp.page.getByRole("option", { name: PROVIDER }).click();

    // The change is now dirty and model_size is no longer applicable.
    await expect(securevuApp.page.getByText(NOT_APPLICABLE)).toBeVisible();
    await expect(securevuApp.page.getByText(UNSAVED)).toBeVisible();
