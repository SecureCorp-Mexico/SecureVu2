...
test.describe("camera ffmpeg input source toggle @medium", () => {
  test("manual input defaults to the manual text field", async ({
    ...
    await installRoutes(securevuApp.page, [
      { path: "rtsp://10.0.0.1:554/video", roles: ["detect"] },
    ]);
    await securevuApp.goto("/settings?page=cameraFfmpeg&camera=front_door");

    await expect(
      securevuApp.page.getByRole("radio", { name: MANUAL_RADIO }),
    ).toBeChecked();
    await expect(
      securevuApp.page.getByRole("textbox", { name: "Input path" }),
  ...
  test("an existing restream path auto-detects into restream mode", async ({
    ...
    await expect(
      securevuApp.page.getByRole("radio", { name: RESTREAM_RADIO }),
    ).toBeChecked();
    // The dropdown is preselected to the matching go2rtc stream.
    await expect(
      securevuApp.page.getByRole("combobox", { name: /go2rtc stream/i }),
  ...
  test("selecting a restream writes the path and preset", async ({
    ...
    const capture = await installRoutes(securevuApp.page, [
      { path: "rtsp://10.0.0.1:554/video", roles: ["detect"] },
    ]);
    await securevuApp.goto("/settings?page=cameraFfmpeg&camera=front_door");

    await securevuApp.page.getByRole("radio", { name: RESTREAM_RADIO }).click();
    ...
    await securevuApp.page.getByPlaceholder("Search streams...").fill("sub");
    await expect(
      securevuApp.page.getByRole("option", { name: "dome_main" }),
    ).toBeHidden();
    await securevuApp.page.getByRole("option", { name: "dome_sub" }).click();

    await securevuApp.page.getByRole("button", { name: "Save" }).click();
  ...
  test("switching a restream back to manual reverts the preset", async ({
    ...
    await securevuApp.page.getByRole("radio", { name: MANUAL_RADIO }).click();
