...
test.describe("camera live playback streams @medium", () => {
  test("switching cameras updates the stream name field", async ({
    ...
    await securevuApp.goto(SETTINGS_URL);

    const streamName = securevuApp.page.getByRole("textbox", {
      name: "Stream name",
    });
    await expect(streamName).toHaveValue("front_door");
    await expect(
      securevuApp.page.getByRole("combobox", { name: "go2rtc stream" }),
    ).toContainText("front_door_main");

    await selectCamera(securevuApp.page, "Backyard");
  ...
  test("typing a new name enables Save without leaving the field", async ({
    ...
    const save = securevuApp.page.getByRole("button", { name: "Save" });
    await expect(save).toBeDisabled();

    const streamName = await streamNameRow(securevuApp.page, "front_door");
    await streamName.click();
    await securevuApp.page.keyboard.press("End");
    await securevuApp.page.keyboard.type("_hd");
  ...
  test("typing through another stream's name keeps both streams", async ({
    ...
    await securevuApp.page.keyboard.press("Backspace");
    await securevuApp.page.keyboard.press("Backspace");
    await securevuApp.page.keyboard.press("Backspace");
    await securevuApp.page.keyboard.press("Backspace");
    await securevuApp.page.keyboard.press("Backspace");
    await expect(streamName).toHaveValue("front");
    await securevuApp.page.keyboard.type("yard");
    await streamName.blur();

    expect(await streamNames(securevuApp.page)).toEqual(["frontyard", "front"]);
  });
