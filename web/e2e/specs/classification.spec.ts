...
test.describe("Classification — model list @medium", () => {
  test("custom models render by name", async ({ securevuApp }) => {
    await securevuApp.installDefaults({
      config: { classification: { custom: CUSTOM_MODELS } },
    });
    await installDatasetRoute(securevuApp, "object_classifier");
    await installDatasetRoute(securevuApp, "state_classifier");
    await securevuApp.goto("/classification");
    await expect(securevuApp.page.locator("#pageRoot")).toBeVisible();
    await expect(securevuApp.page.getByText("object_classifier")).toBeVisible({
      timeout: 10_000,
    });
  });

  test("empty custom map renders without crash", async ({ securevuApp }) => {
    await securevuApp.installDefaults({
  ...
  test("toggling to states view switches the rendered card set", async ({
    ...
    await expect(securevuApp.page.getByText("state_classifier")).toHaveCount(0);

    // Click the "states" toggle. Radix ToggleGroup type="single" uses role="radio".
    const statesToggle = securevuApp.page
...
test.describe("Classification — model detail navigation @medium", () => {
  test("clicking a model card opens ModelTrainingView", async ({
    ...
    await installDatasetRoute(securevuApp, "object_classifier");
    await installDatasetRoute(securevuApp, "state_classifier");
    await installTrainRoute(securevuApp, "object_classifier");
    await securevuApp.goto("/classification");

    const objectCard = securevuApp.page.getByText("object_classifier").first();
    ...
    await expect(
      securevuApp.page.getByRole("button", { name: /back/i }),
...
test.describe("Classification — delete model (desktop) @medium", () => {
  ...
  test("deleting a model fires DELETE + PUT /config/set", async ({
    ...
    await securevuApp.page.route("**/api/config/set", async (route) => {
    ...
    const alert = securevuApp.page.getByRole("alertdialog");
...
test.describe("Classification — admin only @medium", () => {
  test("viewer navigating to /classification is redirected to access-denied", async ({
    ...
    await securevuApp.page.goto("/classification");
    await securevuApp.page.waitForSelector("#pageRoot", { timeout: 10_000 });
    await expect(securevuApp.page).toHaveURL(/\/unauthorized/, {
      timeout: 10_000,
    });
    await expect(
...
test.describe("Classification — mobile @medium @mobile", () => {
  ...
  test("page renders at mobile viewport", async ({ securevuApp }) => {
    await securevuApp.installDefaults({
      config: { classification: { custom: CUSTOM_MODELS } },
    });
    await installDatasetRoute(securevuApp, "object_classifier");
