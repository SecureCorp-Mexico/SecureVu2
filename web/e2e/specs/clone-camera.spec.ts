...
async function openCloneDialog(securevuApp: {
  page: import("@playwright/test").Page;
}) {
  await securevuApp.page
    .getByRole("button", { name: /^Clone settings$/i })
    .click();
  await expect(securevuApp.page.getByRole("dialog")).toBeVisible();
}

async function selectSource(
  securevuApp: { page: import("@playwright/test").Page },
  source: string,
) {
  await securevuApp.page.getByRole("dialog").getByRole("combobox").click();
  await securevuApp.page
...
test.describe("Camera clone dialog @medium @mobile", () => {
  test.beforeEach(async ({ securevuApp }) => {
    await securevuApp.goto("/settings?page=cameraManagement");
    await expect(
      securevuApp.page.getByRole("heading", { name: /Manage Cameras/i }),
    ).toBeVisible();
  });
  ...
  test("opens the dialog from the Clone settings button", async ({
    ...
    await openCloneDialog(securevuApp);

    await expect(
      securevuApp.page.getByRole("dialog").getByText(/Clone camera settings/i),
    ).toBeVisible();

    // The Clone button is disabled until a source (and target) is chosen.
    await expect(
      securevuApp.page.getByRole("button", { name: /^Clone$/i }),
    ).toBeDisabled();
  });
  ...
  test("forces Stream URLs and roles on for new-camera target", async ({
    ...
    await openCloneDialog(securevuApp);
    await selectSource(securevuApp, "Front Door");
    ...
    const streamsLabel = securevuApp.page
      .locator("label")
  ...
  test("issues a single add PUT and shows restart toast for new-camera target", async ({
    ...
    const nameInput = securevuApp.page.getByPlaceholder(
    ...
    await securevuApp.page.getByRole("button", { name: /^Clone$/i }).click();
    ...
    await expect(
      securevuApp.page.getByRole("button", { name: /Restart/i }).first(),
