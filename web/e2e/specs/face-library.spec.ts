...
async function installGroupedFaces(app: SecureVuApp) {
...
async function openGroupedFaceDialog(app: SecureVuApp): Promise<Locator> {
 ...
 */
async function openLibraryDropdown(app: SecureVuApp): Promise<Locator> {
...
test.describe("Face Library — collection selector @high", () => {
  test("selector shows named face collections", async ({ securevuApp }) => {
    await securevuApp.installDefaults({ faces: basicFacesMock() });
    await securevuApp.goto("/faces");
    // Named collections appear in the LibrarySelector dropdown.
    const menu = await openLibraryDropdown(securevuApp);
    await expect(menu.getByText(/alice/i).first()).toBeVisible({
      timeout: 5_000,
    });
  });

  test("empty state renders when no faces exist", async ({ securevuApp }) => {
    await securevuApp.installDefaults({ faces: emptyFacesMock() });
    await securevuApp.goto("/faces");
    await expect(securevuApp.page.locator("#pageRoot")).toBeVisible();
    await expect(
      securevuApp.page.locator('img[src*="/clips/faces/"]'),
    ).toHaveCount(0);
  });

  test("tiles render for each named collection", async ({ securevuApp }) => {
    await securevuApp.installDefaults({ faces: basicFacesMock() });
    await securevuApp.goto("/faces");
...
test.describe("Face Library — upload flow @high", () => {
  ...
  test("Upload button opens the upload dialog", async ({ securevuApp }) => {
    await securevuApp.installDefaults({ faces: basicFacesMock() });
    await securevuApp.goto("/faces");
    ...
    const uploadBtn = securevuApp.page
      .getByRole("button")
...
test.describe("FaceSelectionDialog @high", () => {
  ...
  test("reclassify dropdown selects a name and closes cleanly", async ({
    ...
    const dialog = await openGroupedFaceDialog(securevuApp);
    ...
    const tooltipVisible = await securevuApp.page
  ...
  test("second dropdown open accepts typeahead keyboard input", async ({
    ...
    menu = securevuApp.page
      .locator('[role="menu"], [data-radix-menu-content]')
      .first();
    await expect(menu).toBeVisible({ timeout: 5_000 });

    await securevuApp.page.keyboard.press("c");
    await expect
      .poll(
        async () =>
          securevuApp.page.evaluate(
            () =>
  ...
  test("classifying the last image in a group leaves body interactive", async ({
    ...
    await securevuApp.page.route("**/api/faces", async (route) => {
    ...
    const groupedImage = securevuApp.page
      .locator('img[src*="clips/faces/train/"]')
      .first();
    await expect(groupedImage).toBeVisible({ timeout: 5_000 });
    ...
    const dialog = securevuApp.page
      .getByRole("dialog")
      .filter({
        has: securevuApp.page.locator('img[src*="clips/faces/train/"]'),
      })
      .first();
    await expect(dialog).toBeVisible({ timeout: 5_000 });
    ...
    const librarySelector = securevuApp.page
      .getByRole("button")
      .filter({ hasText: /\(\d+\)/ })
      .first();
...
test.describe("Face Library — mobile @high @mobile", () => {
  ...
  test("mobile library selector dropdown closes cleanly on Escape", async ({
    ...
    // Migrated from radix-overlay-regressions.spec.ts.
    await installGroupedFaces(securevuApp);
    await securevuApp.goto("/faces");

    const selector = securevuApp.page
      .getByRole("button")
      .filter({ hasText: /\(\d+\)/ })
      .first();
