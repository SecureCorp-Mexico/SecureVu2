...
test.describe("Review — severity tabs @critical", () => {
  test("tabs render with Alerts default-on", async ({ securevuApp }) => {
    await securevuApp.goto("/review");
    const review = new ReviewPage(securevuApp.page, !securevuApp.isMobile);
  ...
  test("clicking Detections flips data-state", async ({ securevuApp }) => {
    await securevuApp.goto("/review");
    const review = new ReviewPage(securevuApp.page, !securevuApp.isMobile);
    await expect(review.alertsTab).toBeVisible({ timeout: 10_000 });
  ...
  test("clicking Motion flips data-state", async ({ securevuApp }) => {
    await securevuApp.goto("/review");
    const review = new ReviewPage(securevuApp.page, !securevuApp.isMobile);
    await expect(review.alertsTab).toBeVisible({ timeout: 10_000 });
  ...
  test("switching back to Alerts works", async ({ securevuApp }) => {
    await securevuApp.goto("/review");
    const review = new ReviewPage(securevuApp.page, !securevuApp.isMobile);
    await review.detectionsTab.click();
    await expect(review.detectionsTab).toHaveAttribute("data-state", "on");
...
test.describe("Review — filters (desktop) @critical", () => {
  ...
  test("Cameras popover lists configured camera names", async ({
    ...
    await securevuApp.goto("/review");
    const review = new ReviewPage(securevuApp.page, true);
  ...
  test("Labels are shown inside the General Filter dialog", async ({
    ...
    await securevuApp.goto("/review");
    const filterBtn = securevuApp.page
  ...
  test("Calendar trigger opens a date picker popover", async ({
    ...
    const calendarGrid = securevuApp.page.locator('[role="grid"]').first();
  ...
  test("Show Reviewed switch flips its checked state", async ({
    ...
    await securevuApp.goto("/review");
    const showReviewedSwitch = securevuApp.page.getByRole("switch", {
...
test.describe("Review — timeline (desktop) @critical", () => {
  ...
  test("timeline renders time markers", async ({ securevuApp }) => {
    await securevuApp.goto("/review");
    await expect
      .poll(
...
test.describe("Review — mobile @critical @mobile", () => {
  ...
  test("severity tabs render on mobile", async ({ securevuApp }) => {
    await securevuApp.goto("/review");
    const review = new ReviewPage(securevuApp.page, false);
    await expect(review.alertsTab).toBeVisible({ timeout: 10_000 });
    await expect(review.detectionsTab).toBeVisible();
  });

  test("back navigation returns to Live", async ({ securevuApp }) => {
    await securevuApp.goto("/");
    const base = new BasePage(securevuApp.page, false);
