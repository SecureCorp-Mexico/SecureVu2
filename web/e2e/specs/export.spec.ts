import { test, expect, type SecureVuApp } from "../fixtures/securevu-test";
import {
...
test.describe("Export Page - Delete race @high", () => {
  ...
  test("deleting an export via dropdown→alert→confirm leaves body interactive", async ({
    ...
    await securevuApp.page.route("**/api/export**", async (route) => {
      const payload = deleted ? [] : initialExports;
      await route.fulfill({ json: payload });
    });
    await securevuApp.page.route("**/api/exports/delete", async (route) => {
      deleted = true;
    ...
    await expect(securevuApp.page.getByText("Race - Test Export")).toBeVisible({
      timeout: 5_000,
    });
    ...
    const confirm = securevuApp.page.getByRole("alertdialog");
    ...
    await expect(
      securevuApp.page.getByText("Race - Test Export"),
    ).not.toBeVisible({ timeout: 5_000 });
    await waitForBodyInteractive(securevuApp.page, 5_000);
    await expectBodyInteractive(securevuApp.page);

    // Sanity: another page-level button still responds.
    const newCase = securevuApp.page.getByRole("button", { name: /new case/i });
    await expect(newCase).toBeVisible({ timeout: 3_000 });
    await newCase.click();
    await expect(
      securevuApp.page.getByRole("dialog").filter({ hasText: /create case/i }),
...
test.describe("Export Page - Overview @high", () => {
  test("renders uncategorized exports and case cards from mock data", async ({
    ...
    await expect(
      securevuApp.page.getByText("Front Door - Person Alert"),
    ).toBeVisible();
    await expect(
      securevuApp.page.getByText("Garage - In Progress"),
    ).toBeVisible();
    await expect(
      securevuApp.page.getByText("Package Theft Investigation"),
    ).toBeVisible();
  });

  test("search filters uncategorized exports", async ({ securevuApp }) => {
    await securevuApp.goto("/export");

    const searchInput = securevuApp.page.getByPlaceholder(/search/i).first();
    ...
    await expect(
      securevuApp.page.getByText("Backyard - Car Detection"),
    ).toBeHidden();
    await expect(
      securevuApp.page.getByText("Garage - In Progress"),
    ).toBeHidden();
  ...
  test("new case button opens the create case dialog", async ({
    ...
    await securevuApp.page.getByRole("button", { name: "New Case" }).click();

    await expect(
      securevuApp.page.getByRole("dialog").filter({ hasText: "Create Case" }),
    ).toBeVisible();
    await expect(securevuApp.page.getByPlaceholder("Case name")).toBeVisible();
  });
});
...
test.describe("Export Page - Case Detail @high", () => {
  test("opening a case shows its detail view and associated export", async ({
    ...
    await expect(
      securevuApp.page.getByRole("button", { name: "Add Export" }),
    ).toBeVisible();
    await expect(
      securevuApp.page.getByRole("button", { name: "Edit Case" }),
    ).toBeVisible();
    await expect(
      securevuApp.page.getByRole("button", { name: "Delete Case" }),
    ).toBeVisible();
  });

  test("edit case opens a prefilled dialog", async ({ securevuApp }) => {
    await securevuApp.goto("/export");
    ...
    await securevuApp.page
      ...
      .first()
      .click();
    await securevuApp.page.getByRole("button", { name: "Edit Case" }).click();

    const dialog = securevuApp.page
      .getByRole("dialog")
  ...
  test("add export shows completed uncategorized exports for assignment", async ({
    ...
    await securevuApp.page
      ...
      .first()
      .click();
    await securevuApp.page.getByRole("button", { name: "Add Export" }).click();
  ...
  test("delete case opens a confirmation dialog", async ({ securevuApp }) => {
    await securevuApp.goto("/export");
    ...
    await securevuApp.page
      ...
      .first()
      .click();
    await securevuApp.page.getByRole("button", { name: "Delete Case" }).click();
  ...
  test("delete case can also delete its exports", async ({ securevuApp }) => {
    ...
    await securevuApp.page.route(/\/api\/cases(?:$|\?|\/)/, async (route) => {
    ...
    await securevuApp.page.route("**/api/exports**", async (route) => {
...
test.describe("Export Page - Empty State @high", () => {
  test("renders the empty state when there are no exports or cases", async ({
    ...
    await securevuApp.page.route("**/api/export**", (route) =>
      route.fulfill({ json: [] }),
    );
    await securevuApp.page.route("**/api/exports**", (route) =>
      route.fulfill({ json: [] }),
    );
    await securevuApp.page.route("**/api/cases", (route) =>
      route.fulfill({ json: [] }),
    );
    await securevuApp.page.route("**/api/cases**", (route) =>
      route.fulfill({ json: [] }),
    ...
    await expect(securevuApp.page.getByText("No exports found")).toBeVisible();
  });
});

test.describe("Export Page - Mobile @high @mobile", () => {
  test("mobile can open an export preview dialog", async ({ securevuApp }) => {
    test.skip(!securevuApp.isMobile, "Mobile-only assertion");
...
test.describe("Multi-Review Export @high", () => {
  ...
  test.beforeEach(async ({ securevuApp }) => {
    await routeReviews(securevuApp.page, mockReviews);
  ...
  async function selectTwoReviews(securevuApp: {
    page: import("@playwright/test").Page;
  }) {
    ...
    const reviewItems = securevuApp.page.locator(".review-item");
  ...
  test("selecting two reviews reveals the export button", async ({
    ...
    test.skip(securevuApp.isMobile, "Desktop multi-select flow");

    await securevuApp.goto("/review");

    await selectTwoReviews(securevuApp);

    // Action group replaces the filter bar once items are selected
    await expect(securevuApp.page.getByText(/2.*selected/i)).toBeVisible({
      timeout: 5_000,
    });

    const exportButton = securevuApp.page.getByRole("button", {
      name: /export/i,
    });
  ...
  test("starting an export posts the expected payload and stays on the review page", async ({
    ...
    await securevuApp.page.route("**/api/exports/batch", async (route) => {
    ...
    const viewCase = securevuApp.page.getByRole("link", { name: /view/i });
  ...
  test("mobile opens a drawer (not a dialog) for the multi-review export flow", async ({
    ...
    test.skip(!securevuApp.isMobile, "Mobile-only Drawer assertion");
    ...
    const drawer = securevuApp.page.locator("[data-vaul-drawer]");
  ...
  test("hides export button when more than 50 reviews are selected", async ({
    ...
    test.skip(securevuApp.isMobile, "Desktop select-all keyboard flow");
    ...
    await routeReviews(securevuApp.page, oversizedReviews);
    ...
    await securevuApp.page.keyboard.press("Control+a");
    ...
    await expect(securevuApp.page.getByText(/51.*selected/i)).toBeVisible({
      timeout: 5_000,
    });
    await expect(
      securevuApp.page.getByRole("button", { name: "Mark as reviewed" }),
    ).toBeVisible();
    await expect(
      securevuApp.page.getByRole("button", { name: /^export$/i }),
    ).toHaveCount(0);
  });
...
test.describe("Multi-Camera Export from History @high", () => {
  ...
  async function openRecordingView(securevuApp: SecureVuApp) {
    ...
    await securevuApp.page.route("**/api/*/recordings**", (route) =>
      route.fulfill({ json: [] }),
    );
    await securevuApp.page.route("**/api/recordings/unavailable**", (route) =>
      route.fulfill({ json: [] }),
    );

    await securevuApp.goto(`/review?timestamp=front_door_${playbackTime}`);
  }

  // Desktop opens the export form in a dialog from the Actions menu; mobile
  // opens the same form inside the settings drawer.
  async function openMultiCameraTab(securevuApp: SecureVuApp) {
    await openRecordingView(securevuApp);
    ...
    if (securevuApp.isMobile) {
      ...
      await securevuApp.page.getByRole("button", { name: /^export$/i }).click();
    } else {
      await securevuApp.page
        .getByRole("button", { name: /actions/i })
        .click({ timeout: 15_000 });
      await securevuApp.page.getByRole("menuitem", { name: /export/i }).click();
    }

    const form = securevuApp.page.getByRole("dialog");
  ...
  test("timeline selection renders both export handles on the timeline", async ({
    ...
    await securevuApp.installDefaults();
    const form = await openMultiCameraTab(securevuApp);
    ...
    for (const handle of [".export-start", ".export-end"]) {
      const locator = securevuApp.page.locator(handle);
  ...
  test("the time range picker opens without a configured timezone", async ({
    ...
    await expect(
      securevuApp.page.getByRole("button", { name: /previous month/i }),
  ...
  test("canceling timeline selection reopens the form with the case intact", async ({
    ...
    await securevuApp.page.getByRole("button", { name: /cancel/i }).click();
...
test.describe("Export Page - Active Job Progress @medium", () => {
  test("encoding job renders percent label and progress bar", async ({
    ...
    await securevuApp.page.route("**/api/jobs/export", (route) =>
      route.fulfill({
        json: [
          {
            id: "job-encoding",
  ...
  test("queued job shows queued badge", async ({ securevuApp }) => {
    ...
    await expect(securevuApp.page.getByText("Queued Sample")).toBeVisible();
    await expect(
      securevuApp.page.getByText("Queued", { exact: true }),
    ).toBeVisible();
  });
  ...
  test("stream copy job shows copying label", async ({ securevuApp }) => {
    ...
    await expect(securevuApp.page.getByText("Copy Sample")).toBeVisible();
    await expect(securevuApp.page.getByText(/Copying\s*·\s*80%/)).toBeVisible();
  });

  test("encoding retry job shows retry label", async ({ securevuApp }) => {
    await securevuApp.page.route("**/api/jobs/export", (route) =>
      route.fulfill({
        json: [
          {
    ...
    await expect(securevuApp.page.getByText("Retry Sample")).toBeVisible();
    await expect(
      securevuApp.page.getByText(/Encoding \(retry\)\s*·\s*12%/),
    ).toBeVisible();
  });
});
