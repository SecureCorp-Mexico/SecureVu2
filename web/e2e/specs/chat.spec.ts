 ...
 *
 * Must be called BEFORE securevuApp.goto(). The override also exposes
...
test.describe("Chat — starting state @medium", () => {
  test("empty message list renders ChatStartingState with title and input", async ({
    ...
    await expect(
      securevuApp.page.getByRole("heading", { level: 1 }),
    ).toBeVisible({ timeout: 10_000 });
    await expect(securevuApp.page.getByPlaceholder(/ask/i)).toBeVisible();
    // Four quick-reply buttons from starting_requests.*
    const quickReplies = securevuApp.page.locator(
...
test.describe("Chat — streaming @medium", () => {
  test("submission POSTs to chat/completion with stream: true", async ({
    ...
    await installChatStreamOverride(securevuApp, [
    ...
    const request = await securevuApp.page.evaluate(
      () =>
        (
  ...
  test("NDJSON content chunks accumulate in the assistant bubble", async ({
    ...
    await expect(securevuApp.page.getByText(/Hello, world!/i)).toBeVisible({
      timeout: 10_000,
    });
  });
  ...
  test("tool calls in the chain render a ToolCallsGroup", async ({
    ...
    await expect(securevuApp.page.getByText(/search objects/i)).toBeVisible({
      timeout: 10_000,
    });
    await expect(
      securevuApp.page.getByText(/searching for people/i),
...
test.describe("Chat — stop @medium", () => {
  test("Stop button aborts an in-flight stream and freezes the partial message", async ({
    ...
    await expect(securevuApp.page.getByText(/First chunk\./)).toBeVisible({
      timeout: 10_000,
    });
    ...
    const stopBtn = securevuApp.page
    ...
    await expect(securevuApp.page.getByText(/Third chunk\./)).toHaveCount(0);
  });
});

test.describe("Chat — error @medium", () => {
  test("non-OK response renders an error banner", async ({ securevuApp }) => {
    await installChatStreamOverride(securevuApp, [], { status: 500 });
    await securevuApp.goto("/chat");
    const input = securevuApp.page.getByPlaceholder(/ask/i);
    await expect(input).toBeVisible({ timeout: 10_000 });
    ...
    await expect(
      securevuApp.page.getByRole("alert").filter({
...
test.describe("Chat — attachment chip @medium", () => {
  test("attaching an event renders a ChatAttachmentChip", async ({
    ...
    await expect(securevuApp.page.getByText(/Done\./i)).toBeVisible({
      timeout: 10_000,
    });

    // The paperclip button has aria-label from t("attachment_picker_placeholder")
    // = "Attach an event".
    const paperclip = securevuApp.page
    ...
    const idInput = securevuApp.page
